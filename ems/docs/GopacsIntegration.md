...
- HTTP errors and exceptions on the announcements endpoint **skip the poll and preserve current attributes**, so
  transient API hiccups do not flap the bid status. Any *successful* poll (HTTP 200) that yields no announcement
...
- A *persistent* non-200 (e.g. an invalid API key returning 401, or a sustained outage) keeps the previously selected
...
lifetime — not just the announcements that became active. When an announcement is then *selected* on a poll, a second,
