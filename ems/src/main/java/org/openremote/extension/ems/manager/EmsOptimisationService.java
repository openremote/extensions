...
public class EmsOptimisationService extends RouteBuilder implements ContainerService {
  ...
  protected static final Logger LOG =
      SyslogCategory.getLogger(DATA, EmsOptimisationService.class.getName());
  ...
  public void init(Container container) throws Exception {
    services =
        Services.builder()
            ...
            .withAssetPredictedDatapointService(
  ...
  public void start(Container container) throws Exception {
    ...
    List<EmsEnergyOptimisationAsset> energyOptimisationAssets =
        services
    ...
    if (!energyOptimisationAssets.isEmpty()) {
      List<String> enabledEnergyOptimisationAssetIds =
          energyOptimisationAssets.stream()
              .filter(
                  energyOptimisationAsset ->
                      services
                              .getGatewayService()
                              .getLocallyRegisteredGatewayId(energyOptimisationAsset.getId(), null)
                          == null)
              .filter(
                  energyOptimisationAsset ->
                      !energyOptimisationAsset.getOptimisationDisabled().orElse(false))
              .map(Asset::getId)
              .toList();

      LOG.info(
          String.format(
              "Number of enabled '%s' assets = %s",
              EmsEnergyOptimisationAsset.class.getSimpleName(),
              enabledEnergyOptimisationAssetIds.size()));
    ...
    final Runnable command =
        () -> {
          try {
            ...
            LOG.severe(
                String.format(
                    "assetType='%s', assetId='%s'; Failed to run energy optimisation; Exception: %s",
                    EmsEnergyOptimisationAsset.class.getSimpleName(), assetId, e));
          }
        };

    ScheduledFuture<?> scheduledFuture =
        services.getScheduledExecutorService().scheduleAtFixedRate(command, 1, 1, TimeUnit.MINUTES);
  ...
  private void runOptimisation(String energyOptimisationAssetId) throws Exception {
    ...
    if (triggerTimeMillis != null && currentTimeMillis > triggerTimeMillis) {
      ...
      LocalDate previousDate =
          Instant.ofEpochMilli(triggerTimeMillis - 60000).atZone(zone).toLocalDate();
      ...
      String[] energyOptimisationAssetAttributeNames =
          new String[] {
      ...
      updatePowerLimitProfileAttributes(
          energyOptimisationAsset, energyOptimisationAssetAttributeNames);

      if (gopacsAsset != null) {
        // Update GOPACS asset attributes
        String[] gopacsAttributeNames =
            new String[] {
      ...
      triggerTimeMillis =
          currentTimeMillis - currentTimeMillis % (15 * 60 * 1000) + (15 * 60 * 1000);
      energyOptimisationTimersMap.put(energyOptimisationAssetId, triggerTimeMillis);
    }

    // Run selected optimisation method
    String optimisationMethodName =
        energyOptimisationAsset
            .getOptimisationMethod()
            .orElse(EmsEnergyOptimisationAsset.OptimisationMethodValueType.None)
            .toString();
    OptimisationMethodsLoader optimisationMethodsLoader = new OptimisationMethodsLoader();
    optimisationMethodsLoader.runOptimisationMethod(
        optimisationMethodName, energyOptimisationAssetId, services);
  }
  ...
  private void stopOptimisation(String assetId) {
    ...
    services
        ...
        .purgeValues(
            assetId, EmsEnergyOptimisationAsset.POWER_LIMIT_MAXIMUM_PROFILE_MANUAL.getName());
    services
        .getAssetPredictedDatapointService()
        .purgeValues(
            assetId, EmsEnergyOptimisationAsset.POWER_LIMIT_MINIMUM_PROFILE_MANUAL.getName());
    services
        .getAssetPredictedDatapointService()
        .purgeValues(
            assetId, EmsEnergyOptimisationAsset.POWER_LIMIT_MAXIMUM_PROFILE_TOTAL.getName());
    services
        .getAssetPredictedDatapointService()
        .purgeValues(
            assetId, EmsEnergyOptimisationAsset.POWER_LIMIT_MINIMUM_PROFILE_TOTAL.getName());
  }
    ...
    if (persistenceEvent.getEntity()
      ...
      switch (persistenceEvent.getCause()) {
        ...
        case DELETE -> {
          ...
          stopRedispatchHandlersForAsset(emsGOPACSAsset.getId());
        }
        // Redispatch polling is also managed via attribute events, but an asset merge emits none,
        // so the saved EAN and redispatchEnabled are applied here too.
        case CREATE, UPDATE -> {
          reconcileGopacsHandler(emsGOPACSAsset);
          reconcileRedispatchHandler(emsGOPACSAsset);
        }
      }
    }
  }
  ...
  private void processAttributeEventEmsEnergyOptimisationAsset(AttributeEvent attributeEvent) {
    ...
    EmsEnergyOptimisationAsset energyOptimisationAsset =
        (EmsEnergyOptimisationAsset) services.getAssetStorageService().find(assetId);
    ...
    String logPrefix =
        String.format(
            "assetType='%s', assetId='%s', assetName='%s', attributeName='%s'",
            attributeEvent.getAssetType(),
            attributeEvent.getId(),
            attributeEvent.getAssetName(),
    ...
    if (attributeName.equals(
        EmsEnergyOptimisationAsset.GENERATE_POWER_LIMIT_MAXIMUM_PROFILE_MANUAL_INPUT.getName())) {
      boolean generatePowerLimitMaximumProfileManualInput =
          ...
          .schedule(
              () ->
                  services
                      ...
                      .sendAttributeEvent(
                          new AttributeEvent(assetId, attributeName, false),
                          getClass().getSimpleName()),
              1,
              TimeUnit.SECONDS);

      Double powerLimitMaximumInput =
          energyOptimisationAsset.getPowerLimitMaximumInput().orElse(null);

      if (powerLimitMaximumInput == null) {
        LOG.warning(
            String.format(
                "%s; '%s' attribute value is missing. Unable to generate power limit maximum profile",
                logPrefix, EmsEnergyOptimisationAsset.POWER_LIMIT_MAXIMUM_INPUT.getName()));
        return;
      }

      String powerLimitMaximumProfileManualInput =
          energyOptimisationAsset.getPowerLimitMaximumProfileManualInput().orElse("");

      if (!powerLimitMaximumProfileManualInput.isBlank()) {
        LOG.warning(
            String.format(
                "%s; '%s' attribute is already set. Remove current power limit maximum profile to generate a new power limit profile",
                logPrefix,
                EmsEnergyOptimisationAsset.POWER_LIMIT_MAXIMUM_PROFILE_MANUAL_INPUT.getName()));
        return;
      }
      ...
      services
          ...
          .sendAttributeEvent(
              new AttributeEvent(
                  ...
                  EmsEnergyOptimisationAsset.POWER_LIMIT_MAXIMUM_PROFILE_MANUAL_INPUT,
    ...
    if (attributeName.equals(
        EmsEnergyOptimisationAsset.GENERATE_POWER_LIMIT_MINIMUM_PROFILE_MANUAL_INPUT.getName())) {
      ...
      Double powerLimitMinimumInput =
          energyOptimisationAsset.getPowerLimitMinimumInput().orElse(null);

      if (powerLimitMinimumInput == null) {
        LOG.warning(
            String.format(
                "%s; '%s' attribute value is missing. Unable to generate power limit minimum profile",
                logPrefix, EmsEnergyOptimisationAsset.POWER_LIMIT_MINIMUM_INPUT.getName()));
        return;
      }

      String powerLimitMinimumProfileManualInput =
          energyOptimisationAsset.getPowerLimitMinimumProfileManualInput().orElse("");

      if (!powerLimitMinimumProfileManualInput.isBlank()) {
        LOG.warning(
            String.format(
                "%s; '%s' attribute is already set. Remove current power limit minimum profile to generate a new power limit profile",
                logPrefix,
                EmsEnergyOptimisationAsset.POWER_LIMIT_MINIMUM_PROFILE_MANUAL_INPUT.getName()));
        return;
      }
      ...
      services
          ...
          .sendAttributeEvent(
              new AttributeEvent(
                  energyOptimisationAsset.getId(),
                  EmsEnergyOptimisationAsset.POWER_LIMIT_MINIMUM_PROFILE_MANUAL_INPUT,
                  powerLimitMinimumProfileCsv),
              getClass().getSimpleName());
    }
    ...
    if (attributeName.equals(
      ...
      if (!energyOptimisationAssetsMap.containsKey(assetId)) {
        LOG.warning(
            String.format(
                "%s; Optimisation is disabled. The manual power limit profile has been updated but has not been loaded into the forecast database.",
                logPrefix));
        return;
      }
      ...
      List<ValueDatapoint<?>> powerLimitMaximumProfileManual =
          csvToValueDatapoints(parsedCSV, logPrefix);
      services
          .getAssetPredictedDatapointService()
          .updateValues(
              assetId,
      ...
      if (gopacsAsset != null) {
        ...
        long endTimeMillis =
            startTimeMillis - startTimeMillis % (24 * 60 * 60000) + (8 * 24 * 60 * 60000);
        ...
        List<ValueDatapoint<?>> powerLimitMaximumProfileDayAhead =
            services
                .getAssetPredictedDatapointService()
                .queryDatapoints(
                    gopacsAsset.getId(),
      ...
      List<ValueDatapoint<?>> powerLimitMaximumProfileTotal =
          calculatePowerLimitProfileTotal(powerLimitProfiles, "floor");
      services
          .getAssetPredictedDatapointService()
          .updateValues(
              assetId,
    ...
    if (attributeName.equals(
      ...
      // Convert parsed CSV to database insertable data-points
      List<ValueDatapoint<?>> powerLimitMinimumProfileManual =
          csvToValueDatapoints(parsedCSV, logPrefix);
      services
          .getAssetPredictedDatapointService()
          .updateValues(
      ...
      if (gopacsAsset != null) {
        ...
        // Get power limit profile day-ahead
        List<ValueDatapoint<?>> powerLimitMinimumProfileDayAhead =
            services
                .getAssetPredictedDatapointService()
                .queryDatapoints(
                    gopacsAsset.getId(),
      ...
      if (energyOptimisationAssetsMap.containsKey(assetId)) {
        ...
        services
            ...
            .schedule(
                () ->
                    optimisationMethodsLoader.runOptimisationMethod(
                        optimisationMethodName, assetId, services),
                1,
                TimeUnit.SECONDS);
      }
    }
            ...
            .ifPresent(
                contractedEan -> {
                  ...
                  if (handler != null) {
                    ...
                    LOG.warning(
                        "No redispatch handler running for EAN '"
                            + contractedEan
                            + "' while processing bid confirmation for asset '"
                            + gopacsAsset.getId()
                            + "'");
                    services
                        .getAssetProcessingService()
                        .sendAttributeEvent(
                            new AttributeEvent(
                                gopacsAsset.getId(),
                                EmsGOPACSAsset.REDISPATCH_CONFIRM_BID.getName(),
                                false),
                            getClass().getSimpleName());
                  }
                });
  ...
  private void updatePowerLimitProfileManualForecasts(
    ...
    for (String powerLimitType : powerLimitTypes) {
      ...
      if (powerLimitType.equals("maximum")) {
        powerLimitProfileManualInput =
            energyOptimisationAsset.getPowerLimitMaximumProfileManualInput().orElse("");
        powerLimitProfileManualAttributeName =
            EmsEnergyOptimisationAsset.POWER_LIMIT_MAXIMUM_PROFILE_MANUAL.getName();
      } else {
        powerLimitProfileManualInput =
            energyOptimisationAsset.getPowerLimitMinimumProfileManualInput().orElse("");
  ...
  private void updatePowerLimitProfileAttributes(Asset<?> asset, String[] attributeNames) {
    ...
    for (String attributeName : attributeNames) {
      List<ValueDatapoint<?>> forecastDatapoints =
          services
              .getAssetPredictedDatapointService()
              .queryDatapoints(assetId, attributeName, assetDatapointQuery);
      ...
      long forecastAttributeMillis =
          asset.getAttributes().get(attributeName).flatMap(Attribute::getTimestamp).orElse(0L);

      // Update attribute with new forecast value
      if (forecastDatapoint.getTimestamp() > forecastAttributeMillis
        ...
        services
            .getAssetProcessingService()
            .sendAttributeEvent(
                new AttributeEvent(assetId, attributeName, powerLimit, timestampMillis));
      }
    }
  }

  private void updatePowerLimitProfileTotalForecasts(
      EmsEnergyOptimisationAsset energyOptimisationAsset, EmsGOPACSAsset gopacsAsset) {
    ...
    if (gopacsAsset != null) {
      ...
      powerLimitMaximumProfileTotal =
          calculatePowerLimitProfileTotal(powerLimitMaximumProfiles, "floor");
      powerLimitMinimumProfileTotal =
          calculatePowerLimitProfileTotal(powerLimitMinimumProfiles, "ceil");
    } else {
  ...
  private EmsGOPACSAsset getGopacsAsset(EmsEnergyOptimisationAsset energyOptimisationAsset) {
    ...
    List<EmsGOPACSAsset> gopacsAssets =
        services
            .getAssetStorageService()
            .findAll(
                new AssetQuery()
  ...
  private String generatePowerLimitProfile(double powerLimit) {
    ...
    for (LocalDateTime dateTime = startOfDay;
  ...
  private List<String[]> parseCSV(String csvFile, String delimiter, String logPrefix) {
    ...
    try {
      ...
      try {
        ...
        LOG.warning(
            String.format("%s; Error while closing CSV file reader; Exception: %s", logPrefix, e));
      }
    }
  ...
  private List<ValueDatapoint<?>> csvToValueDatapoints(List<String[]> parsedCSV, String logPrefix) {
    ...
    if (incorrectlyFormattedRows.size() > 0) {
      LOG.warning(
          String.format(
              "%s; Input is incorrectly formatted at row %s;",
              logPrefix, incorrectlyFormattedRows));
    }
  ...
  private List<ValueDatapoint<?>> calculatePowerLimitProfileTotal(
      List<List<ValueDatapoint<?>>> powerLimitProfiles, String floorOrCeil) {
    ...
    if (floorOrCeil.equals("floor")) {
      mergedMap.forEach(
          (timestamp, values) -> {
            ValueDatapoint<?> valueDatapoint =
                new ValueDatapoint<>(timestamp, Collections.min(values));
      ...
      mergedMap.forEach(
          (timestamp, values) -> {
            ValueDatapoint<?> valueDatapoint =
                new ValueDatapoint<>(timestamp, Collections.max(values));
            powerLimitProfileTotal.add(valueDatapoint);
          });
    }
