...
public class GOPACSHandler
    implements UftpPayloadHandler, UftpParticipantService, ParticipantAuthorizationProvider {
  ...
  public static final String DEFAULT_GOPACS_BROKER_URL =
  ...
  public static final String DEFAULT_GOPACS_OAUTH2_URL =
      "https://auth.gopacs-services.eu/realms/gopacs/protocol/openid-connect/token";
  ...
  protected static final UftpSerializer serializer =
      new UftpSerializer(
          new XmlSerializer(),
          new XsdValidator(new XsdSchemaProvider(new XsdFactory(new XsdSchemaFactoryPool()))));
  ...
  public static class Factory {
    ...
    public GOPACSHandler createHandler(
        String contractedEan, String realm, String electricitySupplierAssetId) {
  ...
  protected GOPACSHandler(
    ...
    this.gopacsBrokerUrl =
        container
            .getConfig()
            .getOrDefault(GOPACS_BROKER_URL, DEFAULT_GOPACS_BROKER_URL)
            .replaceAll("/+$", "");
    this.responseDelaySeconds =
        Integer.parseInt(
            container
                .getConfig()
                .getOrDefault(
                    GOPACS_RESPONSE_DELAY_SECONDS, DEFAULT_GOPACS_RESPONSE_DELAY_SECONDS));
    this.flexOfferDelaySeconds =
        Integer.parseInt(
            container
                .getConfig()
                .getOrDefault(
                    GOPACS_FLEX_OFFER_DELAY_SECONDS, DEFAULT_GOPACS_FLEX_OFFER_DELAY_SECONDS));
    ...
    if (clientId == null || clientId.isBlank() || clientSecret == null || clientSecret.isBlank()) {
      throw new RuntimeException(
          GOPACS_CLIENT_ID + " or " + GOPACS_CLIENT_SECRET + " not defined, cannot use GOPACS.");
    }
    ...
    if (TextUtil.isNullOrEmpty(goPacsPrivateKeyFile)) {
      throw new RuntimeException(
          GOPACS_PRIVATE_KEY_FILE + " not defined, can not send use GOPACS.");
    }
    if (!Files.isReadable(Paths.get(goPacsPrivateKeyFile))) {
      throw new RuntimeException(
          GOPACS_PRIVATE_KEY_FILE + " invalid path or file not readable: " + goPacsPrivateKeyFile);
    }
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to initialize GOPACSHandler for ean " + contractedEAN + ".", ex);
    }
    ...
    this.cryptoService =
        new UftpCryptoService(
            participantResolutionService, new LazySodiumFactory(), new LazySodiumBase64Pool());
    ...
    this.uftpSendMessageService =
        new UftpSendMessageService(
            serializer, cryptoService, participantResolutionService, this, uftpValidationService);
  ...
  /**
   * Test-support constructor. Wires the message-processing collaborators directly and skips remote
   * configuration (OAuth client, private-key file) and JAX-RS deployment, so the day-ahead UFTP
   * message flow can be exercised in isolation. Not used in production wiring.
   */
  protected GOPACSHandler(
      String contractedEAN,
      String realm,
  ...
  protected void deploy(Container container) {
    ...
    List<Object> singletons =
        Stream.of(
                ...
                Collections.<Object>singletonList(gopacsServerResource))
            .flatMap(Collection::stream)
            .toList();
    Application application = new WebApplication(container, null, singletons);

    webService.deployJaxRsApplication(
        application,
        DEPLOYMENT_PATH,
        getDeploymentName(contractedEAN),
        0,
        true,
        new CORSConfig()
  ...
  public void notifyNewIncomingMessage(IncomingUftpMessage<? extends PayloadMessageType> message) {
    LOG.info(
        "Received message with conversation ID: "
            + message.payloadMessage().getConversationID()
            + " type "
            + message.payloadMessage().getClass().getSimpleName());
    ...
    LOG.info(
        "Finished processing message with conversation ID: "
            + message.payloadMessage().getConversationID()
            + " type "
            + message.payloadMessage().getClass().getSimpleName());
  }
      ...
      if (message.payloadMessage() instanceof FlexRequestResponse
        ...
        uftpSendMessageService.attemptToSendMessage(
            ...
            new SigningDetails(
                ...
                new UftpParticipant(
                    recipientDomain,
                    UftpRoleInformation.getRecipientRoleBySenderRole(message.sender().role()))));
      }
  ...
  public String getAuthorizationHeader(UftpParticipant uftpParticipant) {
    ...
    if (authorization.isBlank()) {
      LOG.warning(
          "No OAuth2 bearer token available for authorization header of " + uftpParticipant);
    }
  ...
  protected String fetchBearerToken() {
    try {
      try (Response response =
          gopacsAuthResource.getAccessToken(
              "client_credentials", this.clientId, this.clientSecret)) {
        if (response.getStatus() == 200) {
          String responseBody = response.readEntity(String.class);
          OAuth2TokenResponse tokenResponse =
              objectMapper.readValue(responseBody, OAuth2TokenResponse.class);
  ...
  protected void handleFlexRequestMessage(UftpParticipant participant, FlexRequest flexRequest) {
    ...
    for (int i = 0; i < flexRequest.getISPS().size(); i++) {
      ...
      LocalTime start =
          FlexRequestISPTypeHelper.getISPStart(
              flexRequestISPType.getStart(), year, month, day, flexRequest.getTimeZone());
      double importMax =
          flexRequestISPType.getMaxPower() == 0L
              ? 0
              : (double) flexRequestISPType.getMaxPower() / 1000.0F;
      double exportMax =
          flexRequestISPType.getMinPower() == 0L
              ? 0
              : (double) Math.abs(flexRequestISPType.getMinPower()) / 1000.0F;
      LOG.fine("importMax:" + importMax + " exportMax:" + exportMax);
      this.schedulePowerUpdate(
          start, EmsGOPACSAsset.POWER_MAXIMUM_FLEX_REQUEST.getName(), exportMax);
      this.schedulePowerUpdate(
          start, EmsGOPACSAsset.POWER_MINIMUM_FLEX_REQUEST.getName(), exportMax);

      // Correct usage of ZoneId.of instead of ZoneOffset.of
      ZoneId zoneId = ZoneId.of(flexRequest.getTimeZone());
      long startEpochMilli =
          flexRequest.getPeriod().atTime(start).atZone(zoneId).toInstant().toEpochMilli();
    ...
    this.setPredictedDataPoints(
        EmsGOPACSAsset.POWER_MAXIMUM_FLEX_REQUEST.getName(), powerImportMaxGopacsDatapoints);
    this.setPredictedDataPoints(
        EmsGOPACSAsset.POWER_MINIMUM_FLEX_REQUEST.getName(), powerExportMaxGopacsDatapoints);
    LOG.fine("Finished processing Flex Request: " + flexRequest);
  }

  protected void handleFlexOfferResponseMessage(
      UftpParticipant participant, FlexOfferResponse flexOfferResponse) {
  ...
  protected void handleFlexOrderMessage(UftpParticipant participant, FlexOrder flexOrder) {
    ...
    for (int i = 0; i < flexOrder.getISPS().size(); i++) {
      ...
      LocalTime start =
          FlexRequestISPTypeHelper.getISPStart(
              flexOrderISPType.getStart(), year, month, day, flexOrder.getTimeZone());
      ...
      long startEpochMilli =
          flexOrder.getPeriod().atTime(start).atZone(zoneId).toInstant().toEpochMilli();

      powerDatapoints.add(new ValueDatapoint<>(startEpochMilli, power));

      // NOTE: this logic assumes offtake MaxPower is always higher than 0 Watt. If this is not the
      // case this logic needs to be changed
      if (power > 0.0) {
    ...
    if (!powerDatapointsOfftake.isEmpty()) {
      this.setPredictedDataPoints(
          EmsGOPACSAsset.POWER_LIMIT_MAXIMUM_PROFILE_FLEX_ORDER.getName(), powerDatapointsOfftake);
    } else if (!powerDatapointsFeedin.isEmpty()) {
      this.setPredictedDataPoints(
          EmsGOPACSAsset.POWER_LIMIT_MINIMUM_PROFILE_FLEX_ORDER.getName(), powerDatapointsFeedin);
    }

    this.setPredictedDataPoints(
        EmsGOPACSAsset.CURRENT_POWER_FLEX_REQUEST.getName(), powerDatapoints);
  }
  ...
  protected void sendFlexOffer(UftpParticipant recipient, FlexRequest flexRequest) {
    try {
      ...
      for (FlexRequestISPType requestIsp : flexRequest.getISPS()) {
        ...
        if (!firstIsp
            && ((previousPower < 0 && currentPower >= 0)
                || (previousPower >= 0 && currentPower < 0))) {
      ...
      UftpParticipant sender =
          new UftpParticipant(flexRequest.getRecipientDomain(), USEFRoleType.AGR);
  ...
  protected void updatePowerValues(String attributeName, double power) {
    assetProcessingService.sendAttributeEvent(
        new AttributeEvent(electricitySupplierAssetId, attributeName, power),
        getClass().getSimpleName());
  }

  protected void setPredictedDataPoints(
      String attributeName, List<ValueDatapoint<?>> valuesAndTimestamps) {
    assetPredictedDatapointService.updateValues(
        electricitySupplierAssetId, attributeName, valuesAndTimestamps);
  }
   ...
   */
  protected boolean isWithinContractedScope(
      String messageType, String conversationId, String congestionPoint) {
    ...
    LOG.warning(
        "Rejecting "
            ...
            + " "
            + conversationId
            + " for out-of-scope congestion point "
            + congestionPoint
            + " (contracted EAN "
            + contractedEAN
            + ")");
   ...
   * "ean.&lt;code&gt;" (for example "ean.265987182507322951"). GOPACS flex messages always carry
   * the congestion point with the "ean." prefix, whereas the contracted EAN may be configured with
   * or without it; canonicalising both sides keeps the scope check correct either way.
   */
  ...
  protected void processRawMessage(String transportXml) {
    try {
      ...
      // resolves any participant domain, so a validly signed flex message from a participant
      // outside
      // this handler's contracted EAN would otherwise be applied to the asset. Full
      // per-contract/role
      ...
      if (payloadMessage instanceof FlexMessageType flexMessage
          && !isWithinContractedScope(
              payloadMessage.getClass().getSimpleName(),
              payloadMessage.getConversationID(),
              flexMessage.getCongestionPoint())) {
        PayloadMessageType rejection =
            UftpValidationResponseCreator.getResponseForMessage(
                payloadMessage,
                ValidationResult.rejection("CongestionPoint not within contracted scope"));
        UftpParticipant sender = new UftpParticipant(signedMessage);
        UftpParticipant responder =
            new UftpParticipant(
      ...
      var incomingUftpMessage =
          IncomingUftpMessage.create(
  ...
  @Override
  public Optional<UftpParticipantInformation> getParticipantInformation(
      USEFRoleType role, String domain) {
    ...
    if (authorization.isBlank()) {
      LOG.warning(
          "Skipping participant lookup for " + domain + ": no OAuth2 bearer token available");
      return Optional.empty();
    }
    try (Response response =
        gopacsAddressBookResource.fetchParticipantByDomain(authorization, domain)) {
      int status = response != null ? response.getStatus() : -1;
      if (status == 200) {
        ParticipantView view = response.readEntity(ParticipantView.class);
        UftpParticipantInformation info =
            new UftpParticipantInformation(
                view.domain(),
                view.publicKey(),
                this.gopacsBrokerUrl + "/shapeshifter/api/v3/message",
                true);
      ...
      if (status == 404) {
        ...
        LOG.severe(
            "Unexpected status "
                + status
                + " when requesting participant information for "
                + domain);
      }
    } catch (Exception e) {
      Throwable cause = e.getCause() instanceof IOException ? e.getCause() : e;
      LOG.log(
          Level.SEVERE, "Exception when requesting participant information for " + domain, cause);
    }
    return Optional.empty();
  }
}
