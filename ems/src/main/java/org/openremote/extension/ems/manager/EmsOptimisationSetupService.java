...
public class EmsOptimisationSetupService implements ContainerService {
  ...
  public void start(Container container) throws Exception {
    ...
    String[] assetTypes = {EmsEnergyOptimisationSetupAsset.DESCRIPTOR.getName()};
  ...
  private void processAttributeEventEnergyOptimisationSetupAsset(AttributeEvent attributeEvent) {
    ...
    if (attributeName.equals(
        EmsEnergyOptimisationSetupAsset.CREATE_ENERGY_MANAGEMENT_SYSTEM.getName())) {
      boolean checkboxValue = (Boolean) attributeEvent.getValue().orElse(false);

      // Get asset from database
      EmsEnergyOptimisationSetupAsset setupAsset =
          (EmsEnergyOptimisationSetupAsset) assetStorageService.find(assetId);
      ...
      scheduledExecutorService.schedule(
          () ->
              assetProcessingService.sendAttributeEvent(
                  new AttributeEvent(
                      setupAsset.getId(),
                      EmsEnergyOptimisationSetupAsset.CREATE_ENERGY_MANAGEMENT_SYSTEM,
                      false),
      ...
      try {
        ...
        // Add a 1-second delay to ensure the info field is updated after all assets and rules are
        // merged
        scheduledExecutorService.schedule(
            () ->
                assetProcessingService.sendAttributeEvent(
                    new AttributeEvent(
                        setupAsset.getId(),
                        EmsEnergyOptimisationSetupAsset.INFO_FIELD,
                        infoFieldMessage),
                    getClass().getSimpleName()),
            1,
            TimeUnit.SECONDS);
        ...
        LOG.warning(
            String.format(
                "assetName='%s', assetId='%s'; An exception occurred during energy management system creation; Exception: %s",
                setupAsset.getName(), setupAsset.getId(), e));
      }
    }
  }
  ...
  private void updateManagerConfig() {
    ...
    Map<String, Object> assets =
        (Map<String, Object>) pages.computeIfAbsent("assets", k -> new HashMap<>());
    Map<String, Object> viewer =
        (Map<String, Object>) assets.computeIfAbsent("viewer", k -> new HashMap<>());
    Map<String, Object> assetTypes =
        (Map<String, Object>) viewer.computeIfAbsent("assetTypes", k -> new HashMap<>());
  ...
  private String createEnergyManagementSystem(EmsEnergyOptimisationSetupAsset setupAsset) {
    ...
    if (energyManagementSystemName.isBlank()) {
      infoFieldMessage =
          "Energy management system not created:\n" + "  - Set energy management system name";
    ...
    List<EmsEnergyOptimisationAsset> energyOptimisationAssets =
        assetStorageService
            .findAll(
                new AssetQuery()
                    .types(EmsEnergyOptimisationAsset.class)
                    .names(energyOptimisationAssetName))
            .stream()
    ...
    EmsEnergyOptimisationAsset energyOptimisationAsset =
        new EmsEnergyOptimisationAsset(energyOptimisationAssetName);
    ...
    energyOptimisationAsset
        .setOptimisationMethod(
            EmsEnergyOptimisationAsset.OptimisationMethodValueType.EmsOptimisation)
    ...
    energyOptimisationAsset.addOrReplaceAttributes(
        new Attribute<>(
                "powerLimitMaximumFluctuationMargin", ValueType.POSITIVE_NUMBER, powerFluctuation)
            .addOrReplaceMeta(
        ...
        new Attribute<>(
                "powerLimitMinimumFluctuationMargin", ValueType.POSITIVE_NUMBER, powerFluctuation)
            .addOrReplaceMeta(
                new MetaItem<>(MetaItemType.READ_ONLY),
                new MetaItem<>(
                    MetaItemType.UNITS,
    ...
    EmsElectricityBatteryAsset electricityBatteryAsset =
        new EmsElectricityBatteryAsset(energyManagementSystemName + " Battery");
    electricityBatteryAsset
        .setId(UniqueIdentifierGenerator.generateId())
        .setParent(energyOptimisationAsset);
    ...
    if (setupAsset.getIncludeDayAheadForecasts().orElse(false)) {
      EmsDayAheadAsset dayAheadAsset =
          new EmsDayAheadAsset(energyManagementSystemName + " Day ahead forecasts");
      dayAheadAsset
          .setId(UniqueIdentifierGenerator.generateId())
          .setParent(energyOptimisationAsset);

      dayAheadAsset.setCollectTimeForecasts("10:00").setUseTariffDayAheadForecasts(true);
    ...
    try (InputStream inputStream =
        EmsOptimisationService.class.getResourceAsStream(
            "/ems/rules/EmsDemoSimulationRules.groovy")) {
      if (inputStream != null) {
        ...
        rules =
            rules.replaceFirst(
                "linkInputAttributeName1", EmsElectricityBatteryAsset.POWER_SETPOINT.getName());
        rules = rules.replaceFirst("linkOutputAssetId1", energyOptimisationAssetId);
        rules =
            rules.replaceFirst(
                "linkOutputAttributeName1", EmsEnergyOptimisationAsset.POWER_FLEXIBLE.getName());

        // Sum forecasts rule
        rules = rules.replaceFirst("sumInputAssetId1", energyOptimisationAssetId);
        rules =
            rules.replaceFirst(
                "sumInputAttributeName1", EmsEnergyOptimisationAsset.POWER_CONSUMPTION.getName());
        rules = rules.replaceFirst("sumInputAssetId2", energyOptimisationAssetId);
        rules =
            rules.replaceFirst(
                "sumInputAttributeName2", EmsEnergyOptimisationAsset.POWER_PRODUCTION.getName());
        rules = rules.replaceFirst("sumInputAssetId3", energyOptimisationAssetId);
        rules =
            rules.replaceFirst(
                "sumInputAttributeName3", EmsEnergyOptimisationAsset.POWER_FLEXIBLE.getName());
        rules = rules.replaceFirst("sumOutputAssetId", energyOptimisationAssetId);
        rules =
            rules.replaceFirst(
                "sumOutputAttributeName", EmsEnergyOptimisationAsset.POWER_NET.getName());
        ...
        persistenceService.doReturningTransaction(
            entityManager -> entityManager.merge(districtRuleSet));
      }
    } catch (Exception e) {
      LOG.warning(
          String.format(
              "assetName='%s', assetId='%s'; Rules were not created for energy management system '%s'; Exception: %s",
              setupAsset.getName(), setupAsset.getId(), energyOptimisationAssetName, e));
    }
    ...
    try (InputStream inputStream =
        EmsOptimisationService.class.getResourceAsStream(
            "/ems/dashboards/EmsOverviewDashboard.json")) {
      ...
      LOG.warning(
          String.format(
              "assetName='%s', assetId='%s'; Dashboard was not created for energy management system '%s'; Exception: %s",
              setupAsset.getName(), setupAsset.getId(), energyOptimisationAssetName, e));
    }
    ...
    try (InputStream inputStream =
        EmsOptimisationService.class.getResourceAsStream(
            "/ems/dashboards/EmsBatteryDashboard.json")) {
    ...
    infoFieldMessage =
        "Created energy management system \""
            + energyOptimisationAssetName
            + "\":\n"
            + "\n"
    ...
    LOG.info(
        String.format(
            "assetName='%s', assetId='%s'; Created energy management system '%s'",
            setupAsset.getName(), setupAsset.getId(), energyOptimisationAssetName));
