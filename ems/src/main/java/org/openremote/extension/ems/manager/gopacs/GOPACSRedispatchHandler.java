...
/**
 * Handles GOPACS Redispatch (intraday congestion management) by polling for congestion
 * announcements, checking EAN solving effectivity, calculating suggested bid parameters, and
 * managing the operator confirmation workflow.
 */
public class GOPACSRedispatchHandler {
  ...
  public static final String GOPACS_REDISPATCH_POLL_INTERVAL_MINUTES =
      "GOPACS_REDISPATCH_POLL_INTERVAL_MINUTES";
  ...
  protected record EanEffectivityResult(
      String matchedCategory, List<EanSolvingEffectivityDto> effectivities) {}
  ...
  // Bounded LRU set so a long-running handler does not accumulate every announcement ID it has ever
  // seen.
  private final Set<String> recordedAnnouncementIds =
  ...
  protected GOPACSRedispatchHandler(
    ...
    LOG.info(
        "Initialized GOPACSRedispatchHandler for EAN: "
            + contractedEAN
            + " (poll interval: "
            + pollIntervalMinutes
            + " min)");
  ...
  public void startPolling() {
    if (apiKey == null || apiKey.isBlank()) {
      LOG.severe(
          "GOPACS_REDISPATCH_API_KEY not configured; redispatch polling will not start for EAN: "
              + contractedEAN
              + " (the API key is required to resolve EAN effectivity per announcement)");
      return;
    }
  ...
  protected void pollAndProcess() {
    ...
    for (AnnouncementDto announcement : relevant) {
      ...
      if (result != null) {
        ...
        if (selected == null
            || COMPLIANCE_TYPE_MANDATORY.equals(announcement.getComplianceType())) {
    ...
    if (selected == null) {
      LOG.fine(
          "No relevant announcement found for EAN: " + contractedEAN + " after effectivity check");
      updateLastPoll();
      clearAnnouncementAttributes();
      return;
    }
    ...
    if (isNew) {
      LOG.info(
          "New redispatch announcement for EAN "
              + contractedEAN
              + ": id="
              + selected.getId()
              + ", compliance="
              + selected.getComplianceType()
              + ", org="
              + selected.getOrganisationName()
              + ", effectivity="
              + effectivityCategory);
    } else {
    ...
    if (isNew) {
      ...
      sendAttributeEvent(
          EmsGOPACSAsset.REDISPATCH_BID_STATUS.getName(), BID_STATUS_PENDING_CONFIRMATION);
  ...
  /**
   * Returns the fetched announcements, an empty list if the API confirms there are none, or {@code
   * null} if the fetch itself failed (HTTP error / exception). The caller uses the null sentinel to
   * skip processing without wiping current state.
   */
  protected List<AnnouncementDto> fetchAnnouncements() {
    try (Response response =
        announcementResource.fetchAnnouncements(
            null, null, null, ANNOUNCEMENT_TYPE_CONGESTIONMANAGEMENT, ANNOUNCEMENT_STATE_OPEN)) {
  ...
  protected EanEffectivityResult checkEanEffectivity(String announcementId) {
    LOG.fine(
        "Checking EAN effectivity for announcement "
            + announcementId
            + " and EAN "
            + contractedEAN);
    try (Response response =
        eanEffectivityResource.fetchEanSolvingEffectivity(announcementId, apiKey)) {
      if (response.getStatus() == 200) {
        String body = response.readEntity(String.class);
        ...
        for (EanSolvingEffectivityDto effectivity : effectivities) {
          if (effectivity.getEansByCategory() != null) {
            ...
            for (Map.Entry<String, Set<String>> entry :
                effectivity.getEansByCategory().entrySet()) {
        ...
        LOG.warning(
            "Failed to fetch EAN effectivity for announcement "
                + announcementId
                + ": HTTP "
                + response.getStatus());
      }
  ...
  protected void updateAnnouncementAttributes(
      AnnouncementDto announcement, String effectivityCategory) {
    LOG.fine(
        "Updating announcement attributes for announcement "
            + announcement.getId()
            + ", effectivity="
            + effectivityCategory);
    sendAttributeEvent(EmsGOPACSAsset.REDISPATCH_ANNOUNCEMENT_ID.getName(), announcement.getId());
    sendAttributeEvent(
        EmsGOPACSAsset.REDISPATCH_COMPLIANCE_TYPE.getName(), announcement.getComplianceType());
    sendAttributeEvent(
        EmsGOPACSAsset.REDISPATCH_ANNOUNCEMENT_MESSAGE.getName(), announcement.getMessage());
    sendAttributeEvent(
        EmsGOPACSAsset.REDISPATCH_REQUEST_AREA_BUY.getName(),
        announcement.getRequestAreaDescriptionBuyOrders());
    sendAttributeEvent(
        EmsGOPACSAsset.REDISPATCH_REQUEST_AREA_SELL.getName(),
        announcement.getRequestAreaDescriptionSellOrders());

    if (announcement.getProblemPeriod() != null) {
      sendAttributeEvent(
          EmsGOPACSAsset.REDISPATCH_START_TIME.getName(),
          announcement.getProblemPeriod().getStartTime());
      sendAttributeEvent(
          EmsGOPACSAsset.REDISPATCH_END_TIME.getName(),
          announcement.getProblemPeriod().getEndTime());
    } else {
    ...
    if (announcement.getBidValidityPeriod() != null) {
      sendAttributeEvent(
          EmsGOPACSAsset.REDISPATCH_BID_VALIDITY_END.getName(),
          announcement.getBidValidityPeriod().getEndTime());
    } else {
  ...
  protected void clearAnnouncementAttributes() {
    // Check asset state to handle stale attributes after service restart
    // (lastProcessedAnnouncementId is in-memory only)
    if (lastProcessedAnnouncementId != null) {
      LOG.fine(
          "Clearing announcement attributes (previous: "
              + lastProcessedAnnouncementId
              + ") for EAN: "
              + contractedEAN);
    } else {
      EmsGOPACSAsset gopacsAsset = (EmsGOPACSAsset) assetStorageService.find(assetId);
  ...
  protected void storeRequestedPowerProfile(AnnouncementDto announcement) {
    if (announcement.getRemainingProblemProfileInMW() == null
        || announcement.getProblemPeriod() == null) {
    ...
    LOG.fine(
        "Storing "
            + datapoints.size()
            + " power profile data points for announcement "
            + announcement.getId()
            + " (profile MW: "
            + announcement.getRemainingProblemProfileInMW()
            + ")");
    assetPredictedDatapointService.updateValues(
        assetId, EmsGOPACSAsset.REDISPATCH_REQUESTED_POWER.getName(), datapoints);
  }

  protected void recordAnnouncementHistory(
      AnnouncementDto announcement,
      String effectivityCategory,
      List<EanSolvingEffectivityDto> effectivities) {
    try {
      ...
      if (announcement.getRemainingProblemProfileInMW() != null) {
        historyEntry.put(
            "maxRequestedPowerMW",
            announcement.getRemainingProblemProfileInMW().stream()
      ...
      Map<String, Object> historyMap =
          objectMapper.convertValue(historyEntry, new TypeReference<>() {});
  ...
  protected void placeBidOnPlatform(String announcementId, Double bidPrice) {
    ...
    LOG.info(
        "Bid placement placeholder for EAN "
            + contractedEAN
            + ": announcement="
            + announcementId
            + ", price="
            + bidPrice
            + " EUR/MWh. Trading platform integration pending.");
  }
  ...
  private void updateLastPoll() {
    sendAttributeEvent(
        EmsGOPACSAsset.REDISPATCH_LAST_POLL.getName(), timerService.getCurrentTimeMillis());
  }

  private void sendAttributeEvent(String attributeName, Object value) {
    assetProcessingService.sendAttributeEvent(
        new AttributeEvent(assetId, attributeName, value), getClass().getSimpleName());
  }
}
