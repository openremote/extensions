...
public class EmsOptimisation implements OptimisationMethod {
  ...
  private Map<String, Integer> batteryCalculateForecasts(
    ...
    AssetDatapointAllQuery assetDatapointQueryPeriodPredicted =
        new AssetDatapointAllQuery(startTimestampMillis, endTimestampMillis);
    ...
    //        System.out.println("totalPowerConsumptionProductionFlexList = " +
    ...
    for (EmsElectricityBatteryAsset electricityBatteryAsset : electricityBatteryAssets) {
      ...
      for (int i = 0; i < timestampsMillisList.size(); i++) {
        ...
        if (powerLimitMaximum != null) {
          double powerLimitMaximumVirtual =
              powerLimitMaximum * (1 - POWER_LIMIT_MAXIMUM_SAFETY_MARGIN_PERCENTAGE * 0.01);
        ...
        if (powerLimitMinimum != null) {
          double powerLimitMinimumVirtual =
              powerLimitMinimum * (1 - POWER_LIMIT_MINIMUM_SAFETY_MARGIN_PERCENTAGE * 0.01);
      ...
      // chargePowerAvailableTotalList);
      //            System.out.println("dischargePowerAvailableTotalList = " +
      ...
      dischargeEfficiencyBattery =
          dischargeEfficiencyBattery - BATTERY_EFFICIENCY_BUFFER_PERCENTAGE;
      ...
      //            System.out.println("chargePercentageNeededTotalList = " +
      ...
      //            System.out.println("chargePercentageAvailableBatteryList = " +
      // chargePercentageAvailableBatteryList);
      //            System.out.println("dischargePercentageAvailableBatteryList = " +
      ...
      for (int i = 0; i < numberOfDataPoints; i++) {
        ...
        if (getToLimitPercentage > energyLevelPercentageMaximumBattery) {
          ...
          dischargePercentageAvailableBatteryList.set(
              i, dischargePercentageAvailableBatteryList.get(i) - dischargePercentageAvailable);
          ...
          chargePercentageAvailableBatteryList.set(
              i, chargePercentageAvailableBatteryList.get(i) - chargePercentageAvailable);
          getToLimitPercentageList.add(getToLimitPercentage);
        } else {
          break;
        }
      ...
      if (getToLimitPercentage > energyLevelPercentageMaximumBattery
        ...
        // Energy level percentage forecast when whole forecast is outside of battery percentage
        // limits
        energyLevelPredictionList = getToLimitPercentageList;
      } else {
        // Calculate energy level percentage forecast starting from value within battery percentage
        // limits
        ...
        // chargePercentageWantBatteryList);
        //                System.out.println("dischargePercentageWantBatteryList = " +
        // dischargePercentageWantBatteryList);

        List<Double> chargeAndDischargePercentageWantBatteryList = new ArrayList<>();

        for (int i = 0; i < numberOfDataPoints; i++) {
          chargeAndDischargePercentageWantBatteryList.add(
              chargePercentageWantBatteryList.get(i) + dischargePercentageWantBatteryList.get(i));
        }

        //                System.out.println("chargeAndDischargePercentageWantBatteryList = " +
        // chargeAndDischargePercentageWantBatteryList);
        //                System.out.println();
        ...
        //                System.out.println("getToLimitPercentageList = " +
        // getToLimitPercentageList);
        ...
        //                System.out.println("energyLevelPredictionList = " +
        ...
        double energyLevelPredictionMaximum =
            Collections.max(
                energyLevelPredictionList.subList(
                    startRunningSumIndex, energyLevelPredictionList.size()));
        ...
        while (energyLevelPredictionMaximum > energyLevelPercentageMaximumBattery
          ...
          for (int i = intervalEndIndex; i < energyLevelPredictionList.size(); i++) {
            ...
            if (energyLevelPredictionList.get(i) < energyLevelPercentageMinimumBattery) {
              ...
              //                            System.out.println("intervalStartIndex = " +
              // intervalStartIndex);
              //                            System.out.println("intervalEndIndex = " +
              // intervalEndIndex);
          ...
          if (chargeOrDischarge.equals("charge")) {
            ...
            double chargeAvailableInterval =
                energyLevelPercentageMaximumBattery
                    - Collections.max(
                        energyLevelPredictionList.subList(intervalStartIndex, intervalEndIndex));
          ...
          if (energyLevelPredictionValueBefore == energyLevelPredictionList.get(intervalEndIndex)) {
            LOG.warning(
                String.format(
                    "Battery energy level percentage calculation error at index = %s",
                    predictionListIndex));
            //                        System.out.println("ERROR: calculation error at index = " +
            // predictionListIndex);
            break;
          } else if (dt > timeoutMillis) {
            LOG.warning(
                String.format(
                    "Battery energy level percentage calculation timed out at index = %s",
                    predictionListIndex));
            //                        System.out.println("ERROR: calculation timeout at index = " +
            // predictionListIndex);
            break;
          }
        ...
        //                System.out.println("After limits: energyLevelPredictionList = " +
        // energyLevelPredictionList + "\n");
        ...
        Map<Long, Integer> chargeAndDischargeZonesMap =
            calculateTariffChargeAndDischargeZones(
                tariffImportDatapoints, tariffExportDatapoints, 4);

        // Charge zone = 1, discharge zone = -1
        List<Integer> chargeAndDischargeZonesList =
            new ArrayList<>(Collections.nCopies(energyLevelPredictionList.size(), 0));
        ...
        //                System.out.println("chargeAndDischargeZonesList = " +
        // chargeAndDischargeZonesList);

        // Default energy level percentage for each interval
        List<Double> energyLevelPercentageDefaultList =
            new ArrayList<>(
                Collections.nCopies(
                    chargeAndDischargeZonesList.size(), energyLevelPercentageDefault));
        ...
        //                System.out.println("energyLevelPercentageDefaultList = " +
        // energyLevelPercentageDefaultList);

        // TODO: handle debounce interval in forecast
        // Optimise forecast based on optimal tariffs
        for (int i = 1;
            i < chargeAndDischargeZonesList.size() && i < energyLevelPredictionList.size();
            i++) {
          // Only adjust forecast if predicted energy level percentage is outside battery debounce
          // interval
          if (Math.abs(energyLevelPredictionList.get(i) - energyLevelPercentageDefaultList.get(i))
              <= BATTERY_ENERGY_LEVEL_PERCENTAGE_DEBOUNCE_INTERVAL) {
            continue;
          }
          ...
          if (chargeAndDischargeZonesList.get(i) == 1
            ...
            Double energyLevelIntervalMaximum =
                Collections.max(
                    energyLevelPredictionList.subList(i, energyLevelPredictionList.size()));

            double chargeSpaceLeft =
            ...
            if (chargeSpaceLeft > 0) {
              ...
              if (chargeAvailableLeft > 0) {
                chargeAvailableLeft =
                    Math.min(Math.min(chargeSpaceLeft, chargeAvailableLeft), chargeNeeded);

                for (int j = i; j < energyLevelPredictionList.size(); j++) {
                  energyLevelPredictionList.set(
                      j, energyLevelPredictionList.get(j) + chargeAvailableLeft);
                }
              }
            } else {
              ...
              if (energyLevelIntervalMaximum >= energyLevelPercentageMaximumBattery) {
                energyLevelIntervalMaximumIndex =
                ...
                if (chargeNeeded > 0 && chargePercentageWantBatteryList.get(k) == 0) {
                  ...
                  if (chargeAvailableLeft > 0) {
                    double energyLevelIntervalMaximum2 =
                        Collections.max(energyLevelPredictionList.subList(i, k - 1));
            ...
            if (dischargeSpaceLeft < 0) {
              ...
              if (dischargeAvailableLeft < 0) {
                dischargeAvailableLeft =
                    Math.max(Math.max(dischargeSpaceLeft, dischargeAvailableLeft), dischargeNeeded);

                for (int j = i; j < energyLevelPredictionList.size(); j++) {
                  energyLevelPredictionList.set(
                      j, energyLevelPredictionList.get(j) + dischargeAvailableLeft);
                }
              }
            } else {
                ...
                if (dischargeNeeded < 0 && dischargePercentageWantBatteryList.get(k) == 0) {
                  ...
                  if (dischargeAvailableLeft < 0) {
                    double energyLevelIntervalMinimum2 =
                        Collections.min(energyLevelPredictionList.subList(i, k - 1));
      ...
      //            System.out.println("After Tariffs: energyLevelPredictionList = " +
      // energyLevelPredictionList + "\n");
      ...
      //            System.out.println("percentageChangeBatteryList = " +
      ...
      for (int i = 0; i < powerChangeTotalList.size() - 1; i++) {
        totalPowerConsumptionProductionFlexList.set(
            i, totalPowerConsumptionProductionFlexList.get(i) - powerChangeTotalList.get(i));
      }
  ...
  private Map<String, double[]> batteryCalculatePowerFlexibleAvailable(
      List<EmsElectricityBatteryAsset> electricityBatteryAssets) {
    ...
    for (EmsElectricityBatteryAsset electricityBatteryAsset : electricityBatteryAssets) {
      ...
      powerFlexibleAvailable.put(
          ...
          new double[] {chargePowerMaximum, dischargePowerMaximum});
    }
  ...
  private Map<String, Double> batteryCalculatePowerSetpoints(
      ...
      Services services,
      String logPrefix) {
    Map<String, double[]> powerFlexibleAvailable =
    ...
    // Turn off batteries that do not have flexible power available during main power meter
    // disconnect
    ...
    if (powerLimitMaximumProfileTotal != null) {
      powerLimitMaximumBatteries =
          round(
              powerLimitMaximumProfileTotal
                  * (1 - POWER_LIMIT_MAXIMUM_BATTERIES_MARGIN_PERCENTAGE * 0.01),
              3);
    }
    ...
    if (powerLimitMinimumProfileTotal != null) {
      powerLimitMinimumBatteries =
          round(
              powerLimitMinimumProfileTotal
                  * (1 - POWER_LIMIT_MINIMUM_BATTERIES_MARGIN_PERCENTAGE * 0.01),
              3);
    }
    ...
    // TODO: add 15 minute skip protection with forecast update timestamp for robust forecast
    // updating
    ...
    if (powerLimitMaximumProfileTotal != null
        && powerConsumptionVirtual > powerLimitMaximumProfileTotal) {
      Double powerLimitMaximumVirtual =
          round(
              powerLimitMaximumProfileTotal
                  * (1 - POWER_LIMIT_MAXIMUM_SAFETY_MARGIN_PERCENTAGE * 0.01),
              3);
      powerSetpointsNew =
          batteryCalculatePowerSetpointsOnLimitBreach(
              powerFlexibleAvailable, powerConsumptionVirtual, powerLimitMaximumVirtual, "max");
    } else if (powerLimitMinimumProfileTotal != null
        && powerConsumptionVirtual < powerLimitMinimumProfileTotal) {
      Double powerLimitMinimumVirtual =
          round(
              powerLimitMinimumProfileTotal
                  * (1 - POWER_LIMIT_MINIMUM_SAFETY_MARGIN_PERCENTAGE * 0.01),
              3);
      powerSetpointsNew =
          batteryCalculatePowerSetpointsOnLimitBreach(
              powerFlexibleAvailable, powerConsumptionVirtual, powerLimitMinimumVirtual, "min");
    } else {
      powerSetpointsNew =
    ...
    if (powerLimitMaximumProfileTotal != null && powerNet > powerLimitMaximumProfileTotal) {
      ...
      LOG.warning(
          String.format(
              "%s; Power net is %s kW above power limit maximum", logPrefix, powerExceededAmount));
      ...
      if (powerNetNewVirtual > powerLimitMaximumProfileTotal) {
        ...
        LOG.warning(
            String.format(
                ...
                logPrefix, powerReductionShortage));
      }
      ...
      LOG.warning(
          String.format(
              "%s; Power net is %s kW below power limit minimum", logPrefix, powerExceededAmount));
  ...
  private Map<String, Double> batteryCalculatePowerSetpointsOnLimitBreach(
      Map<String, double[]> powerFlexibleAvailable,
      Double powerConsumptionVirtual,
    ...
    for (Map.Entry<String, double[]> entry : powerFlexibleAvailable.entrySet()) {
      ...
      double powerBatteryAvailable =
          isMin
              ? -powerBatteryAvailableChargeDischarge[0]
              : powerBatteryAvailableChargeDischarge[1];
  ...
  private void batteryCheckConnection(
    ...
    // This method checks if a battery is connected based on if the power and energyLevelPercentage
    // attributes update within the active time interval
    String connected =
  ...
  private void batteryCheckSetup(List<EmsElectricityBatteryAsset> electricityBatteryAssets) {
    for (EmsElectricityBatteryAsset electricityBatteryAsset : electricityBatteryAssets) {
      ...
      if (energyLevelPercentage == null) {
        logMessageBatteryConnect.append(
            String.format(" '%s',", EmsElectricityBatteryAsset.ENERGY_LEVEL_PERCENTAGE.getName()));
      }

      if (power == null) {
        logMessageBatteryConnect.append(
            String.format(" '%s',", EmsElectricityBatteryAsset.POWER.getName()));
      }

      if (!logMessageBatteryConnect.isEmpty()) {
        logMessageBatteryConnect.setLength(logMessageBatteryConnect.length() - 1);
        logMessageBatteryConnect.insert(
            0,
            String.format(
                "%s; Can't use battery for flexible power. The following attributes are not connected:",
                logPrefixBattery));
      ...
      if (chargePowerMaximum == null) {
        logMessageBatterySet.append(
            String.format(" '%s',", EmsElectricityBatteryAsset.CHARGE_POWER_MAXIMUM.getName()));
      }

      if (dischargePowerMaximum == null) {
        logMessageBatterySet.append(
            String.format(" '%s',", EmsElectricityBatteryAsset.DISCHARGE_POWER_MAXIMUM.getName()));
      }

      if (energyLevelPercentageMaximum == null) {
        logMessageBatterySet.append(
            String.format(
                " '%s',", EmsElectricityBatteryAsset.ENERGY_LEVEL_PERCENTAGE_MAXIMUM.getName()));
      }

      if (energyLevelPercentageMinimum == null) {
        logMessageBatterySet.append(
            String.format(
                " '%s',", EmsElectricityBatteryAsset.ENERGY_LEVEL_PERCENTAGE_MINIMUM.getName()));
      }

      if (!logMessageBatterySet.isEmpty()) {
        logMessageBatterySet.setLength(logMessageBatterySet.length() - 1);
        logMessageBatterySet.insert(
            0,
            String.format(
                "%s; Can't use battery for flexible power. The following attributes are not set:",
                logPrefixBattery));
      ...
      if (energyLevelPercentageMaximum != null
        ...
        LOG.warning(
            String.format(
                "%s; Can't use battery for flexible power. %s - %s = %s - %s = %s < %s%%)",
                logPrefixBattery,
  ...
  private Map<String, Double> batteryRestoreEnergyLevels(
      EmsEnergyOptimisationAsset energyOptimisationAsset,
      List<EmsElectricityBatteryAsset> electricityBatteryAssets,
      Map<String, double[]> powerFlexibleAvailable,
      Double powerNet,
      Double powerSetpointsCurrentSum,
      Double powerLimitMinimumBatteries,
      Double powerLimitMaximumBatteries,
      Map<String, Integer> batteryEnergyLevelPercentageTargets) {
    Map<String, Double> powerSetpointsNew = new HashMap<>();
    ...
    for (EmsElectricityBatteryAsset electricityBatteryAsset : electricityBatteryAssets) {
      ...
      if (energyLevelPercentage
          < (energyLevelPercentageTarget - BATTERY_ENERGY_LEVEL_PERCENTAGE_DEBOUNCE_INTERVAL)) {
        // Start charging
        double powerSetpointNew = powerFlexibleAvailable.get(electricityBatteryAssetId)[0];
        powerSetpointsNew.put(electricityBatteryAssetId, powerSetpointNew);
      } else if (energyLevelPercentage
          > (energyLevelPercentageTarget + BATTERY_ENERGY_LEVEL_PERCENTAGE_DEBOUNCE_INTERVAL)) {
        // Start discharging
        double powerSetpointNew = powerFlexibleAvailable.get(electricityBatteryAssetId)[1];
        powerSetpointsNew.put(electricityBatteryAssetId, powerSetpointNew);
      } else if (powerSetpointCurrent > 0.0
          && energyLevelPercentage < energyLevelPercentageTarget) {
        // Continue charging
        double powerSetpointNew = powerFlexibleAvailable.get(electricityBatteryAssetId)[0];
        powerSetpointsNew.put(electricityBatteryAssetId, powerSetpointNew);
      } else if (powerSetpointCurrent < 0.0
  ...
  public static List<ValueDatapoint<?>> movingAverage(
      List<ValueDatapoint<?>> dataPoints, int window) {
    ...
    for (int i = 0; i < dataPoints.size(); i++) {
      ...
      if (i >= window - 1) {
        ...
        ValueDatapoint<Double> averaged = new ValueDatapoint<>(original.getTimestamp(), avg);
  ...
  private void updateDayAheadAsset(
    ...
    if (collectTime != null) {
      ...
      long collectTimeMillisStart =
          collectDateTime.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
