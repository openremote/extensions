 ...
 *
 * <p>Delegates to {@link org.openremote.extension.hawkbit.manager.hawkbit.HawkbitRolloutsClient}
...
public interface RolloutResource {
   ...
   *
   * <p>Forces {@code representation=full} so {@code totalTargetsPerStatus} and {@code totalGroups}
   * are populated. hawkBit's default is {@code compact} which omits both.
   */
  @GET
  ...
  Response createRollout(
      @BeanParam RequestParams requestParams, @QueryParam("realm") String realm, JsonNode rollout);

  /** Delete a rollout. */
  @DELETE
  @Path("{id}")
  @RolesAllowed({Constants.WRITE_ADMIN_ROLE})
  Response deleteRollout(
      @BeanParam RequestParams requestParams,
      @QueryParam("realm") String realm,
      @PathParam("id") Long id);
   ...
   *
   * <p>Forces {@code representation=full} so per-group status counters are populated.
   */
  @GET
