...
public class DistributionSetResourceImpl extends HawkbitWebResource
  ...
  public DistributionSetResourceImpl(
  ...
  @Override
  public Response createDistributionSet(
      RequestParams requestParams, String realm, JsonNode distributionSet) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to create firmware distribution set",
        () -> hawkbitFirmwareService.distributionSets().create(distributionSet));
  }

  @Override
  public Response assignDistributionSet(
      RequestParams requestParams, String realm, Long id, Boolean offline, JsonNode targets) {
    requireHawkbitRealmAccess(realm);
    if (targets == null || !targets.isArray() || targets.isEmpty()) {
      throw new WebApplicationException(
          "Assignment requires at least one target", Response.Status.BAD_REQUEST);
    }
    return HawkbitResponseProxy.proxy(
        "Failed to assign firmware distribution set '" + id + "'",
        () -> hawkbitFirmwareService.distributionSets().assignTargets(id, offline, targets));
  }

  @Override
  public Response getDistributionSets(
      RequestParams requestParams, String realm, Integer offset, Integer limit) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to retrieve firmware distribution sets",
  ...
  public Response getDistributionSet(RequestParams requestParams, String realm, Long id) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to retrieve firmware distribution set '" + id + "'",
  ...
  public Response deleteDistributionSet(RequestParams requestParams, String realm, Long id) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to delete firmware distribution set '" + id + "'",
