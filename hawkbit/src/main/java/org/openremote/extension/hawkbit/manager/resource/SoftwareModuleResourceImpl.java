...
public class SoftwareModuleResourceImpl extends HawkbitWebResource
  ...
  public SoftwareModuleResourceImpl(
  ...
  @Override
  public Response createSoftwareModule(
      RequestParams requestParams, String realm, JsonNode softwareModule) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to create firmware software module",
        () -> hawkbitFirmwareService.softwareModules().create(softwareModule));
  }

  @Override
  public Response getSoftwareModules(
      RequestParams requestParams, String realm, Integer offset, Integer limit) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to retrieve firmware software modules",
  ...
  public Response getSoftwareModule(RequestParams requestParams, String realm, Long id) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to retrieve firmware software module '" + id + "'",
  ...
  public Response getSoftwareModuleArtifacts(RequestParams requestParams, String realm, Long id) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to retrieve artifacts for firmware software module '" + id + "'",
        () -> hawkbitFirmwareService.softwareModules().getArtifacts(id));
  }

  @Override
  public Response uploadSoftwareModuleArtifact(
      RequestParams requestParams, String realm, Long id, String filename, List<EntityPart> parts) {
    requireHawkbitRealmAccess(realm);
    try {
      EntityPart filePart =
          parts == null
      ...
      if (filePart == null) {
        throw new WebApplicationException(
            "Missing multipart field 'file'", Response.Status.BAD_REQUEST);
      }
      ...
      String effectiveFilename =
          filename == null || filename.isEmpty() ? submittedFileName : filename;
      String uploadFilename =
          effectiveFilename == null || effectiveFilename.isEmpty()
              ? "artifact.bin"
              : effectiveFilename;
      MultipartFormDataOutput form = new MultipartFormDataOutput();
      form.addFormData(
          "file", inputStream, MediaType.APPLICATION_OCTET_STREAM_TYPE, uploadFilename);
      return HawkbitResponseProxy.proxy(
          "Failed to upload artifact for firmware software module '" + id + "'",
          () ->
  ...
  public Response deleteSoftwareModule(RequestParams requestParams, String realm, Long id) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to delete firmware software module '" + id + "'",
