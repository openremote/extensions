...
public class TargetFilterResourceImpl extends HawkbitWebResource implements TargetFilterResource {

  public TargetFilterResourceImpl(
  ...
  @Override
  public Response getTargetFilters(
    ...
    return HawkbitResponseProxy.proxy(
        "Failed to retrieve firmware target filters",
  ...
  public Response getTargetFilter(RequestParams requestParams, String realm, Long id) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to retrieve firmware target filter '" + id + "'",
        () -> hawkbitFirmwareService.targetFilters().get(id));
  }

  @Override
  public Response createTargetFilter(RequestParams requestParams, String realm, JsonNode filter) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to create firmware target filter",
  ...
  public Response deleteTargetFilter(RequestParams requestParams, String realm, Long id) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
        "Failed to delete firmware target filter '" + id + "'",
  ...
  @Override
  public Response setAutoAssignDS(
      RequestParams requestParams, String realm, Long id, JsonNode request) {
    requireHawkbitRealmAccess(realm);
    return HawkbitResponseProxy.proxy(
