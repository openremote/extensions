 ...
 *
 * <p>Delegates to {@link
 * org.openremote.extension.hawkbit.manager.hawkbit.HawkbitTargetFiltersClient} and returns the
...
public interface TargetFilterResource {
  ...
  Response createTargetFilter(
      @BeanParam RequestParams requestParams, @QueryParam("realm") String realm, JsonNode filter);

  /** Delete a target filter query. */
  @DELETE
  @Path("{id}")
  @RolesAllowed({Constants.WRITE_ADMIN_ROLE})
  Response deleteTargetFilter(
      @BeanParam RequestParams requestParams,
      @QueryParam("realm") String realm,
      @PathParam("id") Long id);
   ...
   *
   * <p>Body matches hawkBit's {@code AutoAssignDistributionSetRequest} shape (distribution-set
   * {@code id} plus optional {@code actionType}).
   */
  @POST
  @Path("{id}/autoAssignDS")
  @Consumes(APPLICATION_JSON)
