...
public final class HawkbitResponseProxy {
   ...
   *
   * <p>The upstream {@link Response} is copied, consumed, and closed by this call.
   */
  ...
  private static Response copyResponse(Response response) {
    try (response) {
      ...
      throw new WebApplicationException(
          "Failed to copy hawkBit response", e, Response.Status.BAD_GATEWAY);
    }
  }

  /**
   * Replaces {@link java.util.function.Supplier} to allow checked exceptions to propagate without
   * {@code UndeclaredThrowableException} wrapping.
   */
  @FunctionalInterface
