 ...
 *
 * <p>Delegates to {@link
 * org.openremote.extension.hawkbit.manager.hawkbit.HawkbitDistributionSetsClient} and returns the
...
public interface DistributionSetResource {
  ...
  Response createDistributionSet(
   ...
   *
   * <p>{@code targets} must be a non-empty JSON array. Empty or non-array bodies return {@code
   * 400}.
   *
   * <p>{@code offline=true} records the assignment as already-installed and skips the controller
   * download step. Use this when the target was flashed out-of-band.
   */
  @POST
  @Path("{id}/assign")
  @Consumes(APPLICATION_JSON)
  @Produces(APPLICATION_JSON)
  @RolesAllowed({Constants.WRITE_ADMIN_ROLE})
  Response assignDistributionSet(
      @BeanParam RequestParams requestParams,
      @QueryParam("realm") String realm,
  ...
  @Path("{id}")
  @RolesAllowed({Constants.WRITE_ADMIN_ROLE})
  Response deleteDistributionSet(
      @BeanParam RequestParams requestParams,
      @QueryParam("realm") String realm,
      @PathParam("id") Long id);
}
