 ...
 *
 * <p>Delegates to {@link org.openremote.extension.hawkbit.manager.hawkbit.HawkbitTargetsClient} and
 * returns the upstream response body unchanged.
 */
...
public interface TargetResource {
   ...
   *
   * <p>{@code q} is a hawkBit RSQL filter (e.g. {@code name==foo}), not free-text search.
   */
  @GET
   ...
   *
   * <p>"Assigned" is the DS the server has scheduled. See {@link #getInstalledDs} for what the
   * target has confirmed installed.
   */
  @GET
  @Path("{id}/assignedDS")
  @Produces(APPLICATION_JSON)
  @RolesAllowed({Constants.READ_ADMIN_ROLE})
  Response getAssignedDs(
      @BeanParam RequestParams requestParams,
      @QueryParam("realm") String realm,
      @PathParam("id") String id);
   ...
   *
   * <p>"Installed" reflects the target's last confirmation. See {@link #getAssignedDs} for what the
   * server has scheduled.
   */
  @GET
  @Path("{id}/installedDS")
  @Produces(APPLICATION_JSON)
  @RolesAllowed({Constants.READ_ADMIN_ROLE})
  Response getInstalledDs(
      @BeanParam RequestParams requestParams,
      @QueryParam("realm") String realm,
      @PathParam("id") String id);
   ...
   *
   * <p>Each entry carries the {@code messages} the controller reported over the DDI feedback
   * channel (e.g. OTA progress or failure detail), plus the reported status code.
   */
  @GET
   ...
   *
   * <p>{@code force=true} bypasses the cancel-confirmation handshake with the controller. The
   * target stays in an unknown state until its next poll.
   */
  @DELETE
  @Path("{id}/actions/{actionId}")
  @RolesAllowed({Constants.WRITE_ADMIN_ROLE})
  Response cancelAction(
      @BeanParam RequestParams requestParams,
      @QueryParam("realm") String realm,
      @PathParam("id") String id,
      @PathParam("actionId") Long actionId,
