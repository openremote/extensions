...
public class ForecastWindService extends RouteBuilder implements ContainerService {
  ...
  protected static final Logger LOG =
      SyslogCategory.getLogger(DATA, ForecastWindService.class.getName());
  ...
  public void configure() throws Exception {
    from(PERSISTENCE_TOPIC)
        ...
        .process(
            exchange ->
                processAssetChange(
                    (PersistenceEvent<ElectricityProducerWindAsset>)
  ...
  public void start(Container container) throws Exception {
    ...
    List<ElectricityProducerWindAsset> electricityProducerWindAssets =
        assetStorageService
            .findAll(new AssetQuery().types(ElectricityProducerWindAsset.class))
            .stream()
            .map(asset -> (ElectricityProducerWindAsset) asset)
            .filter(
                electricityProducerWindAsset ->
                    electricityProducerWindAsset.isIncludeForecastWindService().orElse(false)
  ...
  protected synchronized void processElectricityProducerWindAssetAttributeEvent(
    ...
    if (attributeEvent
        .getName()
        .equals(ElectricityProducerWindAsset.INCLUDE_FORECAST_WIND_SERVICE.getName())) {
      ...
      ElectricityProducerWindAsset asset =
          (ElectricityProducerWindAsset) assetStorageService.find(attributeEvent.getId());

      if (asset != null
          && asset.isIncludeForecastWindService().orElse(false)
          && asset.getLocation().isPresent()) {
    ...
    if (attributeEvent
        .getName()
        .equals(ElectricityProducerWindAsset.SET_ACTUAL_WIND_VALUE_WITH_FORECAST.getName())) {
      // Get latest asset from storage
      ElectricityProducerWindAsset asset =
          (ElectricityProducerWindAsset) assetStorageService.find(attributeEvent.getId());
      ...
      if (asset.getPower().orElse(0d) == 0d && asset.getPowerForecast().orElse(0d) != 0d) {
        assetProcessingService.sendAttributeEvent(
            new AttributeEvent(
                asset.getId(),
                ElectricityProducerWindAsset.POWER,
                asset.getPowerForecast().orElse(0d)),
  ...
  protected void processAssetChange(
      PersistenceEvent<ElectricityProducerWindAsset> persistenceEvent) {
  ...
  protected void startCalculation(ElectricityProducerWindAsset electricityProducerWindAsset) {
    ...
    calculationFutures.put(
        electricityProducerWindAsset.getId(),
    ...
    try (Response response =
      ...
      if (response != null && response.getStatus() == 200) {
        ...
        WeatherForecastResponseModel weatherForecastResponseModel =
            response.readEntity(WeatherForecastResponseModel.class);

        double currentPower =
            calculatePower(electricityProducerWindAsset, weatherForecastResponseModel.current);

        assetProcessingService.sendAttributeEvent(
            new AttributeEvent(
                electricityProducerWindAsset.getId(),
                ElectricityProducerAsset.POWER_FORECAST.getName(),
                -currentPower),
            getClass().getSimpleName());
        ...
        if (electricityProducerWindAsset.isSetActualWindValueWithForecast().orElse(false)) {
          assetProcessingService.sendAttributeEvent(
              new AttributeEvent(
                  electricityProducerWindAsset.getId(),
                  ElectricityProducerAsset.POWER.getName(),
                  -currentPower),
              getClass().getSimpleName());
        }
        ...
        for (WeatherForecastModel weatherForecastModel : weatherForecastResponseModel.getList()) {
          ...
          LocalDateTime timestamp =
              Instant.ofEpochMilli(weatherForecastModel.getTimestamp())
