...
public class EnergyOptimisationService extends RouteBuilder implements ContainerService {
  ...
  public void start(Container container) throws Exception {
    ...
    List<EnergyOptimisationAsset> energyOptimisationAssets =
        assetStorageService.findAll(new AssetQuery().types(EnergyOptimisationAsset.class)).stream()
  ...
  public void configure() throws Exception {
    from(PERSISTENCE_TOPIC)
        ...
        .process(
            exchange ->
                processAssetChange(
                    (PersistenceEvent<EnergyOptimisationAsset>)
                        exchange.getIn().getBody(PersistenceEvent.class)));
  }

  @Override
  public void stop(Container container) throws Exception {
    new ArrayList<>(assetOptimisationInstanceMap.keySet()).forEach(this::stopOptimisation);
  }
  ...
  protected void processAttributeEvent(AttributeEvent attributeEvent) {
    OptimisationInstance optimisationInstance =
        assetOptimisationInstanceMap.get(attributeEvent.getId());
    ...
    if ((attributeName.equals(ElectricityChargerAsset.VEHICLE_CONNECTED.getName())
            || attributeName.equals(ElectricVehicleAsset.CHARGER_CONNECTED.getName()))
      ...
      if (forceChargeAssetIds.remove(attributeEvent.getId())) {
        LOG.fine(
            "Previously force charged asset has now been disconnected so clearing force charge flag: "
    ...
    if (attributeName.equals(ElectricityStorageAsset.FORCE_CHARGE.getName())) {
      ...
      if (!(asset instanceof ElectricityStorageAsset)) {
        LOG.fine(
            "Request to force charge asset will be ignored as asset not found or is not of type '"
                + ElectricityStorageAsset.class.getSimpleName()
                + "': "
                + attributeEvent.getId());
        return;
      }
      ...
      if (attributeEvent.getValue().orElse(null) == AttributeExecuteStatus.REQUEST_START) {
        ...
        LOG.fine(
            "Request to force charge asset '"
                + attributeEvent.getId()
                + "': attempting to set powerSetpoint="
                + powerImportMax);

        if (forceChargeAssetIds.contains(attributeEvent.getId())) {
          LOG.fine(
              "Request to force charge asset will be ignored as force charge already requested for asset: "
                  + storageAsset);
          return;
        }
        ...
        if (currentEnergyLevel >= maxEnergyLevel) {
          LOG.fine(
              "Request to force charge asset will be ignored as asset is already at or above maxEnergyLevel: "
                  + storageAsset);
          return;
        }
        ...
        assetProcessingService.sendAttributeEvent(
            new AttributeEvent(
                storageAsset.getId(), ElectricityAsset.POWER_SETPOINT, powerImportMax),
            getClass().getSimpleName());
        assetProcessingService.sendAttributeEvent(
            new AttributeEvent(
                storageAsset.getId(),
                ElectricityStorageAsset.FORCE_CHARGE,
                AttributeExecuteStatus.RUNNING),
            getClass().getSimpleName());

      } else if (attributeEvent.<AttributeExecuteStatus>getValue().orElse(null)
          == AttributeExecuteStatus.REQUEST_CANCEL) {

        if (forceChargeAssetIds.remove(attributeEvent.getId())) {
          LOG.info("Request to cancel force charge asset: " + storageAsset.getId());
          assetProcessingService.sendAttributeEvent(
              new AttributeEvent(storageAsset.getId(), ElectricityAsset.POWER_SETPOINT, 0d),
              getClass().getSimpleName());
          assetProcessingService.sendAttributeEvent(
              new AttributeEvent(
                  storageAsset.getId(),
                  ElectricityStorageAsset.FORCE_CHARGE,
                  AttributeExecuteStatus.CANCELLED),
              getClass().getSimpleName());
        }
      }
    }
  ...
  protected synchronized void processOptimisationAssetAttributeEvent(
      OptimisationInstance optimisationInstance, AttributeEvent attributeEvent) {
    ...
    if (attributeEvent.getName().equals(EnergyOptimisationAsset.OPTIMISATION_DISABLED.getName())) {
      ...
      if (!disabled
          && assetOptimisationInstanceMap.containsKey(
              optimisationInstance.optimisationAsset.getId())) {
        // Nothing to do here
        return;
      } else if (disabled
          && !assetOptimisationInstanceMap.containsKey(
    ...
    EnergyOptimisationAsset asset =
        (EnergyOptimisationAsset) assetStorageService.find(attributeEvent.getId());
  ...
  protected synchronized void startOptimisation(EnergyOptimisationAsset optimisationAsset) {
    ...
    try {
      EnergyOptimiser optimiser =
          new EnergyOptimiser(intervalSize, ((double) financialWeighting) / 100);

      long periodSeconds = (long) (optimiser.intervalSize * 60 * 60);

      if (periodSeconds < 300) {
        throw new IllegalStateException(
            "Optimiser interval size is too small (minimum is 5 mins) for asset: "
                + optimisationAsset.getId());
      }
      ...
      Duration startDuration =
          Duration.between(
              ...
              optimisationStartTime.plus(offsetSeconds, ChronoUnit.SECONDS));

      ScheduledFuture<?> optimisationFuture =
          scheduleOptimisation(optimisationAsset.getId(), optimiser, startDuration, periodSeconds);
      assetOptimisationInstanceMap.put(
          optimisationAsset.getId(),
          new OptimisationInstance(optimisationAsset, optimiser, optimisationFuture));

      // Execute first optimisation at the period that started previous to now
      LOG.finest(
          getLogPrefix(optimisationAsset.getId())
              + "Running first optimisation for time '"
              + formatter.format(optimisationStartTime));
      ...
      executorService.execute(
          () -> {
            try {
              ...
              LOG.log(
                  Level.SEVERE,
                  "Failed to run energy optimiser for asset: " + optimisationAsset.getId(),
                  e);
            }
          });
    } catch (Exception e) {
  ...
  protected synchronized void stopOptimisation(String optimisationAssetId) {
    OptimisationInstance optimisationInstance =
        assetOptimisationInstanceMap.remove(optimisationAssetId);
  ...
  /**
   * Schedules execution of the optimiser at the start of the interval window with up to 30s of
   * offset randomness added so that multiple optimisers don't all run at exactly the same instance;
   * the interval execution times are calculated relative to the hour. e.g. a 0.25h intervalSize
   * (15min) would execute at NN:00+offset, NN:15+offset, NN:30+offset, NN:45+offset...It is
   * important that intervals coincide with any change in supplier tariff so that the optimisation
   * works effectively.
   */
  protected ScheduledFuture<?> scheduleOptimisation(
      String optimisationAssetId,
      EnergyOptimiser optimiser,
      Duration startDuration,
      long periodSeconds)
    ...
    if (optimiser == null) {
      throw new IllegalStateException(
          "Optimiser instance not found for asset: " + optimisationAssetId);
    }
    ...
    return scheduledExecutorService.scheduleAtFixedRate(
        () -> {
          try {
            runOptimisation(
                optimisationAssetId,
                Instant.ofEpochMilli(timerService.getCurrentTimeMillis())
                    .truncatedTo(ChronoUnit.MINUTES));
          } catch (Exception e) {
            LOG.log(
                Level.SEVERE,
                "Failed to run energy optimiser for asset: " + optimisationAssetId,
                e);
          }
        },
  ...
  protected void checkTimeoutAndThrow(String optimisationAssetId, long startTimeMillis)
      throws TimeoutException {
    long runtime = timerService.getCurrentTimeMillis() - startTimeMillis;
    if (runtime > OPTIMISATION_TIMEOUT_MILLIS) {
      String logMsg =
          getLogPrefix(optimisationAssetId)
              + "Optimisation has been running for "
              + runtime
              + "ms, timeout is at "
  ...
  /**
   * Runs the optimisation routine for the specified time; it is important that this method does not
   * throw an exception as it will cancel the scheduled task thus stopping future optimisations.
   */
  protected void runOptimisation(String optimisationAssetId, Instant optimisationTime)
      throws Exception {
    OptimisationInstance optimisationInstance =
        assetOptimisationInstanceMap.get(optimisationAssetId);
    ...
    LOG.finest(
        getLogPrefix(optimisationAssetId)
            + "Running for time '"
            + formatter.format(optimisationTime));
    ...
    LOG.finest(
        getLogPrefix(optimisationAssetId)
            + "Fetching child assets of type '"
            + ElectricitySupplierAsset.class.getSimpleName()
            + "'");

    List<ElectricitySupplierAsset> supplierAssets =
        assetStorageService
            .findAll(
                new AssetQuery()
    ...
    if (supplierAssets.size() != 1) {
      LOG.warning(
          getLogPrefix(optimisationAssetId)
              ...
              + ElectricitySupplierAsset.class.getSimpleName()
              + " asset with a '"
              + ElectricitySupplierAsset.TARIFF_IMPORT.getName()
              + "' attribute but found: "
              + supplierAssets.size());
      return;
    }
    ...
    LOG.finest(
        getLogPrefix(optimisationAssetId)
            + "Fetching optimisable child assets of type '"
            + ElectricityStorageAsset.class.getSimpleName()
            + "'");
                            ...
                            Collections.singletonList(
                                new LogicGroup<>(
                                    ...
                                    new AttributePredicate(
                                        ElectricityStorageAsset.SUPPORTS_IMPORT.getName(),
                                        new BooleanPredicate(true)),
                                    new AttributePredicate(
                                        ElectricityStorageAsset.SUPPORTS_EXPORT.getName(),
                                        new BooleanPredicate(true)))),
                            new AttributePredicate()
                                .name(
                                    new StringPredicate(
                                        ElectricityAsset.POWER_SETPOINT.getName())))))
            .stream()
    ...
    optimisableStorageAssets =
        optimisableStorageAssets.stream()
            .filter(
                asset -> {
                  ...
                  if (forceChargeAssetIds.contains(asset.getId())) {
                    LOG.finest(
                        "Optimisable asset was requested to force charge so it won't be optimised: "
                            + asset.getId());
                    @SuppressWarnings("OptionalGetWithoutIsPresent")
                    Attribute<Double> powerAttribute =
                        asset.getAttribute(ElectricityAsset.POWER).get();
                    double[] powerLevels =
                        get24HAttributeValues(
                            asset.getId(),
                            powerAttribute,
                            optimiser.getIntervalSize(),
                            intervalCount,
                            optimisationTime);
                    ...
                    if (currentEnergyLevel >= maxEnergyLevel) {
                      LOG.info(
                          "Force charged asset has reached maxEnergyLevelPercentage so stopping charging: "
                              + asset.getId());
                      forceChargeAssetIds.remove(asset.getId());
                      assetProcessingService.sendAttributeEvent(
                          new AttributeEvent(
                              asset.getId(), ElectricityStorageAsset.POWER_SETPOINT, 0d),
                          getClass().getSimpleName());
                      assetProcessingService.sendAttributeEvent(
                          new AttributeEvent(
                              asset.getId(),
                              ElectricityStorageAsset.FORCE_CHARGE,
                              AttributeExecuteStatus.COMPLETED),
                          getClass().getSimpleName());
                    }
                    return false;
                  }
                  ...
                  if (asset instanceof ElectricityChargerAsset) {
                    ...
                    return finalOptimisableStorageAssets.stream()
                        .noneMatch(
                            a -> {
                              if (a instanceof ElectricVehicleAsset
                                  && a.getParentId().equals(asset.getId())) {
                                // Take the lowest power max from vehicle or charger
                                double vehiclePowerImportMax =
                                    a.getPowerImportMax().orElse(Double.MAX_VALUE);
                                double vehiclePowerExportMax =
                                    a.getPowerExportMax().orElse(Double.MAX_VALUE);
                                double chargerPowerImportMax =
                                    asset.getPowerImportMax().orElse(Double.MAX_VALUE);
                                double chargerPowerExportMax =
                                    asset.getPowerExportMax().orElse(Double.MAX_VALUE);
                                double smallestPowerImportMax =
                                    Math.min(vehiclePowerImportMax, chargerPowerImportMax);
                                double smallestPowerExportMax =
                                    Math.min(vehiclePowerExportMax, chargerPowerExportMax);

                                if (smallestPowerImportMax < vehiclePowerImportMax) {
                                  LOG.fine(
                                      "Reducing vehicle power import max due to connected charger limit: vehicle="
                                          + a.getId()
                                          + ", oldPowerImportMax="
                                          + vehiclePowerImportMax
                                          + ", newPowerImportMax="
                                          + smallestPowerImportMax);
                                  a.setPowerImportMax(smallestPowerImportMax);
                                }
                                if (smallestPowerExportMax < vehiclePowerExportMax) {
                                  LOG.fine(
                                      "Reducing vehicle power Export max due to connected charger limit: vehicle="
                                          + a.getId()
                                          + ", oldPowerExportMax="
                                          + vehiclePowerExportMax
                                          + ", newPowerExportMax="
                                          + smallestPowerExportMax);
                                  a.setPowerExportMax(smallestPowerExportMax);
                                }
                                LOG.finest(
                                    "Excluding charger from optimisable assets and child vehicle will be used instead: "
                                        + asset.getId());
                                return true;
                              }
                              return false;
            ...
            .sorted(
                Comparator.comparingInt(
                    asset -> asset.getEnergyLevelSchedule().map(schedule -> 0).orElse(1)))
            .collect(Collectors.toList());
    ...
    if (optimisableStorageAssets.isEmpty()) {
      LOG.warning(
          getLogPrefix(optimisationAssetId)
              + "Expected at least one optimisable '"
              + ElectricityStorageAsset.class.getSimpleName()
              + " asset with a '"
              + ElectricityAsset.POWER_SETPOINT.getName()
    ...
    if (LOG.isLoggable(Level.FINEST)) {
      LOG.finest(
          getLogPrefix(optimisationAssetId)
              + "Found optimisable child assets of type '"
              + ElectricityStorageAsset.class.getSimpleName()
              + "': "
              + optimisableStorageAssets.stream()
                  .map(Asset::getId)
    ...
    LOG.finest(
        getLogPrefix(optimisationAssetId)
            + "Fetching plain consumer and producer child assets of type '"
            + ElectricityProducerAsset.class.getSimpleName()
            + "', '"
            + ElectricityConsumerAsset.class.getSimpleName()
            + "', '"
            + ElectricityStorageAsset.class.getSimpleName()
            + "'");
    ...
    assetStorageService
        .findAll(
            new AssetQuery()
                ...
                .attributes(
                    new AttributePredicate()
                        .name(new StringPredicate(ElectricityAsset.POWER.getName()))))
        // .stream()
    ...
    // Get power of storage assets that don't support neither import or export (treat them as plain
    // consumers/producers)
    List<ElectricityStorageAsset> plainStorageAssets =
        assetStorageService
            .findAll(
                new AssetQuery()
                    .recursive(true)
    ...
    // Exclude chargers with a power value != 0 and a child vehicle with a power value != 0 (avoid
    // double counting - vehicle takes priority)
    plainStorageAssets.stream()
        .filter(
            asset -> {
              if (asset instanceof ElectricityChargerAsset) {
                // Check if it has a child vehicle asset also check optimisable assets as child
                // vehicle could be in there
                        ...
                        .noneMatch(
                            a -> {
                              if (a instanceof ElectricVehicleAsset
                                  && a.getParentId().equals(asset.getId())) {
                                LOG.finest(
                                    "Excluding charger from plain consumer/producer calculations to avoid double counting power: "
                                        + asset.getId());
                                return true;
                              }
                              return false;
    ...
    if (LOG.isLoggable(Level.FINEST)) {
      LOG.finest(
          getLogPrefix(optimisationAssetId)
              + "Found plain consumer and producer child assets count="
              + count.get());
    ...
    double[] costsImport =
        get24HAttributeValues(
            supplierAsset.getId(),
            supplierAsset.getAttribute(ElectricitySupplierAsset.TARIFF_IMPORT).orElse(null),
            optimiser.getIntervalSize(),
            intervalCount,
            optimisationTime);
    double[] costsExport =
        get24HAttributeValues(
            supplierAsset.getId(),
            supplierAsset.getAttribute(ElectricitySupplierAsset.TARIFF_EXPORT).orElse(null),
            optimiser.getIntervalSize(),
            intervalCount,
            optimisationTime);
    ...
    if (financialWeightingImport < 1d || financialWeightingExport < 1d) {
      double[] carbonImport =
          get24HAttributeValues(
              supplierAsset.getId(),
              supplierAsset.getAttribute(ElectricitySupplierAsset.CARBON_IMPORT).orElse(null),
              optimiser.getIntervalSize(),
              intervalCount,
              optimisationTime);
      double[] carbonExport =
          get24HAttributeValues(
              supplierAsset.getId(),
              supplierAsset.getAttribute(ElectricitySupplierAsset.CARBON_EXPORT).orElse(null),
              optimiser.getIntervalSize(),
              intervalCount,
              optimisationTime);
      ...
      LOG.finest(
          getLogPrefix(optimisationAssetId)
              + "Adjusting costs to include some carbon weighting, financialWeightingImport="
              + financialWeightingImport
              + ", financialWeightingExport="
              + financialWeightingExport);

      for (int i = 0; i < costsImport.length; i++) {
        costsImport[i] =
            (financialWeightingImport * costsImport[i])
                + ((1 - financialWeightingImport) * carbonImport[i]);
        costsExport[i] =
            (financialWeightingExport * costsExport[i])
                + ((1 - financialWeightingExport) * carbonExport[i]);
      }
    }

    if (LOG.isLoggable(Level.FINEST)) {
      LOG.finest(
          getLogPrefix(optimisationAssetId) + "Import costs: " + Arrays.toString(costsImport));
      LOG.finest(
          getLogPrefix(optimisationAssetId) + "Export costs: " + Arrays.toString(costsExport));
    }

    // Savings variables
    List<String> obsoleteUnoptimisedAssetIds =
        new ArrayList<>(optimisationInstance.unoptimisedStorageAssetEnergyLevels.keySet());
    ...
    // Optimise storage assets with priority on storage assets with an energy schedule (already
    // sorted above)
    ...
    for (ElectricityStorageAsset storageAsset : optimisableStorageAssets) {
      ...
      LOG.finest(
          getLogPrefix(optimisationAssetId)
              + "Optimising power set points for storage asset: "
              + storageAsset);

      if (!supportsExport && !supportsImport) {
        LOG.finest(
            getLogPrefix(optimisationAssetId)
                + "Storage asset doesn't support import or export: "
                + storageAsset.getId());
        continue;
      }
      ...
      if (!hasSetpoint) {
        LOG.info(
            getLogPrefix(optimisationAssetId)
                + "Storage asset has no '"
                + ElectricityStorageAsset.POWER_SETPOINT.getName()
                + "' attribute so cannot be controlled: "
                + storageAsset.getId());
        continue;
      }
      ...
      if (energyCapacity <= 0d || energyLevel < 0) {
        LOG.info(
            getLogPrefix(optimisationAssetId)
                + "Storage asset has no capacity or energy level so cannot import or export energy: "
                + storageAsset.getId());
        continue;
      }
      ...
      double energyLevelMax =
          Math.min(
              energyCapacity,
              ((double) storageAsset.getEnergyLevelPercentageMax().orElse(100) / 100)
                  * energyCapacity);
      double energyLevelMin =
          Math.min(
              energyCapacity,
              ((double) storageAsset.getEnergyLevelPercentageMin().orElse(0) / 100)
                  * energyCapacity);
      ...
      boolean hasEnergyMinRequirement =
          energyLevelMin > 0 || energyLevelScheduleOptional.isPresent();
      double powerExportMax =
          storageAsset.getPowerExportMax().map(power -> -1 * power).orElse(Double.MIN_VALUE);
      double powerImportMax = storageAsset.getPowerImportMax().orElse(Double.MAX_VALUE);
      int[][] energySchedule =
          energyLevelScheduleOptional
              .map(
                  dayArr ->
                      Arrays.stream(dayArr)
                          .map(
                              hourArr ->
                                  Arrays.stream(hourArr).mapToInt(i -> i != null ? i : 0).toArray())
                          .toArray(int[][]::new))
              .orElse(null);
      ...
      if (energySchedule != null) {
        LOG.finest(
            getLogPrefix(optimisationAssetId)
                + "Applying energy schedule for storage asset: "
                + storageAsset.getId());
        optimiser.applyEnergySchedule(
            energyLevelMins,
            energyLevelMaxs,
            energyCapacity,
            energySchedule,
      ...
      Function<Integer, Double> powerImportMaxCalculator =
          interval -> interval == 0 && !isConnected ? 0 : powerImportMax;
      Function<Integer, Double> powerExportMaxCalculator =
          interval -> interval == 0 && !isConnected ? 0 : powerExportMax;

      if (hasEnergyMinRequirement) {
        LOG.finest(
            getLogPrefix(optimisationAssetId)
                + "Normalising min energy requirements for storage asset: "
                + storageAsset.getId());
        optimiser.normaliseEnergyMinRequirements(
            energyLevelMins, powerImportMaxCalculator, powerExportMaxCalculator, energyLevel);
        if (LOG.isLoggable(Level.FINEST)) {
          LOG.finest(
              getLogPrefix(optimisationAssetId)
                  + "Min energy requirements for storage asset '"
                  + storageAsset.getId()
                  + "': "
                  + Arrays.toString(energyLevelMins));
        }
      }
      ...
      if (setpoints != null) {
        ...
        for (int i = 0; i < powerNets.length; i++) {
          ...
          if (i == 0) {
            if (!storageAssetConnected(storageAsset)) {
              LOG.finest(
                  "Optimised storage asset not connected so interval 0 will not be counted or actioned: "
                      + storageAsset.getId());
              setpoints[i] = 0;
              continue;
            }
            ...
            if (setpoints[i] > 0) {
              financialCost +=
                  storageAsset.getTariffImport().orElse(0d) * setpoints[i] * intervalSize;
            } else {
              financialCost +=
                  storageAsset.getTariffExport().orElse(0d) * -1 * setpoints[i] * intervalSize;
            }
          }
        ...
        // Push the setpoints into the prediction service for the storage asset's setpoint attribute
        // and set current setpoint
        List<ValueDatapoint<?>> valuesAndTimestamps =
            IntStream.range(1, setpoints.length)
                .mapToObj(
                    i ->
                        new ValueDatapoint<>(
                            optimisationTime
                                .plus(periodSeconds * i, ChronoUnit.SECONDS)
                                .toEpochMilli(),
                            setpoints[i]))
                .collect(Collectors.toList());

        assetPredictedDatapointService.updateValues(
            storageAsset.getId(), ElectricityAsset.POWER_SETPOINT.getName(), valuesAndTimestamps);
      }

      assetProcessingService.sendAttributeEvent(
          new AttributeEvent(
              storageAsset.getId(),
              ElectricityAsset.POWER_SETPOINT,
              setpoints != null ? setpoints[0] : null),
          getClass().getSimpleName());

      // Update unoptimised power for this asset
      obsoleteUnoptimisedAssetIds.remove(storageAsset.getId());
      double assetUnoptimisedPower =
          getStorageUnoptimisedImportPower(
              optimisationInstance,
              optimisationAssetId,
              storageAsset,
              maxEnergyLevelMin,
              Math.max(0, powerImportMax - unoptimisedPower));
      unoptimisedPower += assetUnoptimisedPower;
      unoptimisedFinancialCost +=
          storageAsset.getTariffImport().orElse(0d) * assetUnoptimisedPower * intervalSize;
    }

    // Clear out un-optimised data for not found assets
    obsoleteUnoptimisedAssetIds.forEach(
        optimisationInstance.unoptimisedStorageAssetEnergyLevels.keySet()::remove);

    // Calculate and store savings data
    carbonCost =
        (powerNets[0] >= 0
                ? supplierAsset.getCarbonImport().orElse(0d)
                : -1 * supplierAsset.getCarbonExport().orElse(0d))
            * powerNets[0]
            * intervalSize;
    financialCost +=
        (powerNets[0] >= 0
                ? supplierAsset.getTariffImport().orElse(0d)
                : -1 * supplierAsset.getTariffExport().orElse(0d))
            * powerNets[0]
            * intervalSize;
    unoptimisedCarbonCost =
        (unoptimisedPower >= 0
                ? supplierAsset.getCarbonImport().orElse(0d)
                : -1 * supplierAsset.getCarbonExport().orElse(0d))
            * unoptimisedPower
    ...
    LOG.info(
        getLogPrefix(optimisationAssetId)
            + "Current interval financial saving = "
            + financialSaving);
    LOG.info(
        getLogPrefix(optimisationAssetId) + "Current interval carbon saving = " + carbonSaving);
    ...
    assetProcessingService.sendAttributeEvent(
        new AttributeEvent(
            optimisationAssetId, EnergyOptimisationAsset.FINANCIAL_SAVING, financialSaving),
        getClass().getSimpleName());
    assetProcessingService.sendAttributeEvent(
        new AttributeEvent(
            optimisationAssetId, EnergyOptimisationAsset.CARBON_SAVING, carbonSaving),
        getClass().getSimpleName());
  }
  ...
  protected boolean isElectricityGroupAsset(Asset<?> asset) {
    ...
    Class<?> assetClass =
        ValueUtil.getAssetDescriptor(((GroupAsset) asset).getChildAssetType().orElse(null))
  ...
  protected double[] get24HAttributeValues(
      String assetId,
      Attribute<Double> attribute,
      double intervalSize,
      int intervalCount,
      Instant optimisationTime) {
    ...
    if (attribute.hasMeta(MetaItemType.HAS_PREDICTED_DATA_POINTS)) {
      ...
      List<ValueDatapoint<?>> predictedData =
      ...
      if (predictedData.size() != values.length) {
        LOG.warning(
            "Returned predicted data point count does not match interval count: Ref="
                + ref
                + ", expected="
                + values.length
                + ", actual="
        ...
        IntStream.range(0, predictedData.size())
            .forEach(
                i -> {
                  if (predictedData.get(i).getValue() != null) {
                    values[i] = (double) (Object) predictedData.get(i).getValue();
                  } else {
                    // Average previous and next values to fill in gaps (goes up to 5 back and
                    // forward) - this fixes
                    // issues with resolution differences between stored predicted data and
  ...
  /**
   * Returns the power setpoint calculator for the specified asset (for producers power demand will
   * only ever be negative, for consumers it will only ever be positive and for storage assets that
   * support export (i.e. supports producer and consumer) it can be positive or negative at a given
   * interval. For this to work the supplied parameters should be updated when the system changes
   * and not replaced so that references maintained by the calculator are valid and up to date.
   */
  protected double[] getStoragePowerSetpoints(
    ...
    Function<Integer, Double> energyLevelCalculator =
        interval ->
            energyLevel
                + IntStream.range(0, interval)
                    .mapToDouble(j -> powerSetpoints[j] * optimiser.getIntervalSize())
                    .sum();

    // If asset supports exporting energy (V2G, battery storage, etc.) then need to determine if
    // there are
    // opportunities to export energy to save/earn, taking into consideration the cost of exporting
    // from this asset
    if (supportsExport) {
      LOG.finest(
          getLogPrefix(optimisationAssetId)
              + "Storage asset supports export so calculating export cost and power levels for each interval: "
              + storageAsset.getId());
      // Find intervals that save/earn by exporting energy from this storage asset by looking at
      // power levels
      BiFunction<Integer, Double, double[]> exportOptimiser =
          optimiser.getExportOptimiser(
              powerNets,
              exportPowerLimits,
              costImports,
              costExports,
              storageAsset.getTariffExport().orElse(0d));
      exportCostAndPower =
          IntStream.range(0, intervalCount)
              .mapToObj(it -> exportOptimiser.apply(it, powerExportMax))
              .toArray(double[][]::new);
    }

    // If asset supports importing energy then need to determine if there are opportunities to
    // import energy to
    // save/earn, taking into consideration the cost of importing to this asset, also need to ensure
    // that min
    // energy demands are met.
    if (supportsImport) {
      LOG.finest(
          getLogPrefix(optimisationAssetId)
              + "Storage asset supports import so calculating export cost and power levels for each interval: "
              + storageAsset.getId());
      BiFunction<Integer, double[], double[]> importOptimiser =
          optimiser.getImportOptimiser(
              powerNets,
              importPowerLimits,
              costImports,
              costExports,
              storageAsset.getTariffImport().orElse(0d));
      importCostAndPower =
          IntStream.range(0, intervalCount)
              .mapToObj(it -> importOptimiser.apply(it, new double[] {0d, powerImportMax}))
              .toArray(double[][]::new);

      boolean hasEnergyMinRequirement =
          Arrays.stream(normalisedEnergyLevelMins).anyMatch(el -> el > 0);

      if (hasEnergyMinRequirement) {
        LOG.finest(
            getLogPrefix(optimisationAssetId)
                + "Applying imports to achieve min energy level requirements for storage asset: "
                + storageAsset.getId());
        optimiser.applyEnergyMinImports(
            importCostAndPower,
            normalisedEnergyLevelMins,
            powerSetpoints,
        ...
        if (LOG.isLoggable(Level.FINEST)) {
          LOG.finest(
              getLogPrefix(optimisationAssetId)
                  + "Setpoints to achieve min energy level requirements for storage asset '"
                  + storageAsset.getId()
                  + "': "
                  + Arrays.toString(powerSetpoints));
        }
      }
    }

    optimiser.applyEarningOpportunities(
        importCostAndPower,
        exportCostAndPower,
        normalisedEnergyLevelMins,
        energyLevelMaxs,
        ...
        energyLevelCalculator,
        powerImportMaxCalculator,
        powerExportMaxCalculator);

    if (LOG.isLoggable(Level.FINEST)) {
      LOG.finest(
          getLogPrefix(optimisationAssetId)
              + "Calculated earning opportunity power set points for storage asset '"
              + storageAsset.getId()
              + "': "
              + Arrays.toString(powerSetpoints));
    }
  ...
  /**
   * Gets the un-optimised import power for the first (current) interval for the supplied storage
   * asset
   */
  protected double getStorageUnoptimisedImportPower(
      ...
      String optimisationAssetId,
      ElectricityStorageAsset storageAsset,
      double energyLevelTarget,
      double remainingPowerCapacity) {
    ...
    double energyLevel =
        optimisationInstance.unoptimisedStorageAssetEnergyLevels.get(storageAsset.getId()) != null
            ? optimisationInstance.unoptimisedStorageAssetEnergyLevels.get(storageAsset.getId())
            : storageAsset.getEnergyLevel().orElse(-1d);

    if (energyLevel < 0) {
      LOG.finest(
          getLogPrefix(optimisationAssetId)
              + "Storage asset has no energy level so cannot calculate un-optimised power demand: "
              + storageAsset.getId());
      return 0;
    }
