...
public class EnergyOptimiser {
  ...
  public EnergyOptimiser(double intervalSize, double financialWeighting)
  ...
  //    /**
  //     * Function to calculate the power requirements for the given storage asset at each interval
  // in the window
  //     * based on the provided power demand requirements and potential cost saving at each
  // interval so for each interval
  //     * a an array of [powerDemand, gridCost] is required. For a given interval a positive value
  // means importing
  //     * power and negative means exporting power:
  //     * <ul>
  //     * <li>powerDemand = The net demand of suppliers - consumers - higher priority supplier
  // capabilities</li>
  ...
  //    public Function<double[][], double[]> getStoragePowerCalculator(ElectricityStorageAsset
  // storageAsset) {
  //
  ...
  //            double costExport = financialWeighting * tariffExport + (1d-financialWeighting) *
  // carbonExport;
  //            double costImport = financialWeighting * tariffImport + (1d-financialWeighting) *
  // carbonImport;
  ...
  // */
  //
  //            // Order intervals based on potential income [gridCost+storageImportCost] < 0
  // (smallest first)
  //            Integer[] indexesSortedIncome = IntStream.range(0,
  // powerExportsAndSavings.length).boxed().toArray(Integer[]::new);
  //            Arrays.sort(
  //                indexesSortedIncome,
  ...
  //
  //
  //            // Order intervals based on potential cost saving [gridCost-storageExportCost]
  // (largest first)
  //            Integer[] indexesSortedSaving = IntStream.range(0,
  // powerExportsAndSavings.length).boxed().toArray(Integer[]::new);
  //            Arrays.sort(
  //                indexesSortedSaving,
  //                Comparator.<Integer>comparingDouble((i) -> powerExportsAndSavings[i][1] -
  // costExport).reversed()
  //            );
  //
  //
  //            // Find an interval earlier than each ordered export interval that costs less than
  ...
  //                Integer[] indexesSortedGridCost = IntStream.range(0,
  // powerExportIndex).boxed().toArray(Integer[]::new);
  //                Arrays.sort(
  //                    indexesSortedGridCost,
  //                    Comparator.comparingDouble((i) -> powerExportsAndSavings[i][1])
  //                );
  //
  //                // Go through intervals allocating required power demand until interval reaches
  // powerImportMax or storage is full
  ...
  /**
   * Will take the supplied 24x7 energy schedule percentages and energy level min/max values and
   * apply them to the supplied energyLevelMins also adjusting for any intervalSize difference. The
   * energy schedule should be in UTC time.
   */
  public void applyEnergySchedule(
      double[] energyLevelMins,
      double[] energyLevelMaxs,
      double energyCapacity,
      int[][] energyLevelSchedule,
    ...
    // Extract the schedule for the next 24 hour period starting at current hour plus 1 (need to
    // attain energy level by the time the hour starts)
    ...
    if (intervalSize <= 1d) {
      ...
      for (i = 0; i < schedule.length; i++) {
        ...
        energyLevelMins[(hourIntervals * i)] =
            Math.min(
                energyLevelMaxs[hourIntervals * i],
                Math.max(energyLevelMins[hourIntervals * i], schedule[i]));
      }
    } else {
      ...
      for (i = 0; i < hourIntervals; i++) {
        ...
        energyLevelMins[i] =
            Math.min(
                ...
                Math.max(
                    energyLevelMins[i],
                    java.util.Arrays.stream(schedule, (i * takeSize), (i * takeSize) + takeSize)
                        .max()
                        .orElse(0)));
      }
    }
  ...
  /**
   * Adjusts the supplied energyLevelMin values to match the physical characteristics (i.e. the
   * charge and discharge rates).
   */
  public void normaliseEnergyMinRequirements(
      double[] energyLevelMins,
    ...
    Function<Integer, Double> previousEnergyLevelCalculator =
        i -> (i == 0 ? energyLevel : energyLevelMins[i - 1]);
        ...
        .forEach(
            i -> {
              ...
              if (energyDelta > 0) {
                ...
                // May need to increase earlier min values until there is no energy deficit with
                // previous interval
                // If we reach interval 0 and there is still a deficit then need to reduce this
                // energy level
                for (int j = i; j >= 0; j--) {
                  double previousMin =
                      energyLevelMins[j] - (powerImportMaxCalculator.apply(j) * intervalSize);
                ...
                for (int j = i; j < intervalCount; j++) {
                  ...
                  double min =
                      previousEnergyLevelCalculator.apply(j)
                          + (powerExportMaxCalculator.apply(j) * intervalSize);
  ...
  public void applyEnergyMinImports(
      double[][] importCostAndPower,
      double[] energyLevelMins,
    ...
    IntStream.range(0, get24HourIntervalCount())
        .forEach(
            i -> {
  ...
  /**
   * Creates earlier imports between fromInterval (inclusive) and toInterval (exclusive) in order to
   * meet min energy level requirement at the specified interval based on the provided energy level
   * at the start of fromInterval.
   */
  public void retrospectiveEnergyAllocator(
      double[][] importCostAndPower,
      double[] energyLevelMins,
      double[] powerSetpoints,
      BiFunction<Integer, double[], double[]> importOptimiser,
    ...
    boolean canMeetDeficit =
        IntStream.range(fromInterval, toInterval)
                .mapToDouble(
                    i -> Math.min(powerImportMaxCalculator.apply(i), importCostAndPower[i][2]))
                .sum()
            >= energyDeficit;
    boolean morePowerAvailable =
        !canMeetDeficit
            && IntStream.range(fromInterval, toInterval)
                .mapToObj(i -> importCostAndPower[i][2] < powerImportMaxCalculator.apply(i))
                .anyMatch(b -> b);
          ...
          .forEach(
              i -> {
                ...
                if (importCostAndPower[i][2] < powerImportMax) {
                  importCostAndPower[i] =
                      importOptimiser.apply(i, new double[] {0d, powerImportMax});
                }
              });
    }

    // Sort import intervals by cost (lowest to highest)
    List<Pair<Integer, double[]>> sortedImportCostAndPower =
        IntStream.range(fromInterval, toInterval)
            .mapToObj(i -> new Pair<>(i, importCostAndPower[i]))
            .sorted(Comparator.comparingDouble(pair -> pair.value[0]))
    ...
    while (energyDeficit > 0 && i < sortedImportCostAndPower.size()) {
      ...
      importPower =
          importCostAndPower[i][0] < 0 ? importPower : Math.min(importPower, requiredPower);
  ...
  /**
   * Will find the best earning opportunity for each interval (import or export) and will then try
   * to apply them in chronological order (reallocating earlier import/exports if it cost
   * beneficial). The powerSetpoints will be updated as a result.
   */
  public void applyEarningOpportunities(
      double[][] importCostAndPower,
      double[][] exportCostAndPower,
      double[] energyLevelMins,
      double[] energyLevelMaxs,
      double[] powerSetpoints,
      Function<Integer, Double> energyLevelCalculator,
    ...
    double[][] primary =
        importCostAndPower != null ? importCostAndPower : exportCostAndPower; // Never null
    double[][] secondary = importCostAndPower != null ? exportCostAndPower : null; // Could be null

    List<Pair<Integer, double[]>> earningOpportunities =
        IntStream.range(0, primary.length)
            .mapToObj(
                i -> {
                  if (secondary == null) {
    ...
    if (LOG.isLoggable(Level.FINEST)) {
      earningOpportunities.forEach(
          op ->
              LOG.finest(
                  "Earning opportunity: interval="
                      + op.key
                      + ", cost="
                      + op.value[0]
                      + ", powerMin="
    ...
    // Go through each earning opportunity and determine if it can be utilised without breaching the
    // energy min
    // levels
    for (Pair<Integer, double[]> earningOpportunity : earningOpportunities) {
      ...
      if (isImportOpportunity(
          costAndPower, powerSetpoints[interval], interval, powerImportMaxCalculator)) {
        ...
        applyImportOpportunity(
            ...
            interval);
      } else if (isExportOpportunity(
          costAndPower, powerSetpoints[interval], interval, powerExportMaxCalculator)) {
  ...
  protected boolean isImportOpportunity(
      double[] costAndPower,
      double powerSetpoint,
      int interval,
      Function<Integer, Double> powerImportMaxCalculator) {
    return costAndPower[2] > 0
        && powerSetpoint >= 0
        && powerSetpoint < Math.min(powerImportMaxCalculator.apply(interval), costAndPower[2]);
  }

  protected boolean isExportOpportunity(
      double[] costAndPower,
      double powerSetpoint,
      int interval,
      Function<Integer, Double> powerExportMaxCalculator) {
    return costAndPower[1] < 0
        && powerSetpoint <= 0
        && powerSetpoint > Math.max(powerExportMaxCalculator.apply(interval), costAndPower[1]);
  }

  /**
   * Tries to apply the maximum import power as defined in the importCostAndPower at the specified
   * interval taking into consideration the maximum power and energy levels; if there is
   * insufficient power or energy capacity at the interval then an earlier cost effective export
   * opportunity will be attempted to offset the requirement. The powerSetpoints will be updated as
   * a result.
   */
  public void applyImportOpportunity(
    ...
    if (energySpace < energySpaceMax && exportCostAndPower != null) {
      ...
      LOG.finest(
          "Looking for earlier export opportunities to maximise on this import opportunity: space="
              + energySpace
              + ", max="
              + energySpaceMax);
      int i = interval - 1;
      ...
      while (energySpace < energySpaceMax && j < pastOpportunities.size()) {
        ...
        double expPowerMax =
            Math.max(powerExportMaxCalculator.apply(pastInterval), pastCostAndPower[1]);
        ...
        double energySurplus =
            energyLevelMins[pastInterval] - energyLevelCalculator.apply(pastInterval);
        energySurplus = Math.max(energySurplus, energySpace - energySpaceMax);

        // We have spare energy capacity and power check if we don't violate energy min for any
        // future exports
        k = pastInterval;
        while (k < powerSetpoints.length
            && energySurplus < 0
            && energySurplus <= energySurplusMin) {
          ...
          if (energySurplus <= 0) {
            LOG.finest(
                "Earlier export opportunity would violate future energy min level: interval="
                    + j
                    + ", futureInterval="
                    + k);
          }
        ...
        if (expPowerCapacity < 0 && expPowerCapacity < pastCostAndPower[2]) {
          ...
          LOG.finest(
              "Earlier export opportunity identified: interval="
                  + pastInterval
                  + ", power="
                  + expPowerCapacity);
        }
    ...
    if (energySpace > 0 && energySpace >= energySpaceMin) {
      ...
      pastIntervalPowerDeltas.forEach(
          intervalAndDelta -> powerSetpoints[intervalAndDelta.key] += intervalAndDelta.value);
      ...
      LOG.finest(
          "Applied import earning opportunity: set point="
              + powerSetpoints[interval]
              + " (delta: "
              + powerCapacity
              + ")");
    }
  ...
  /**
   * Tries to apply the maximum export power as defined in the exportCostAndPower at the specified
   * interval taking into consideration the maximum power and energy levels; if there is
   * insufficient power or energy capacity at the interval then an earlier cost effective import
   * opportunity will be attempted to offset the requirement. The powerSetpoints will be updated as
   * a result.
   */
  public void applyExportOpportunity(
      double[][] importCostAndPower,
      double[][] exportCostAndPower,
      double[] energyLevelMins,
      double[] energyLevelMaxs,
    ...
    while (k < powerSetpoints.length && energySurplus > 0 && energySurplus >= energySurplusMin) {
      ...
      // The following is an attempt to make use of an earning opportunity that would violate future
      // energy limits
      ...
      //                // Try and allocate an import between this future point and the earning
      // opportunity to prevent this deficit
      //                int l = k - 1;
      //                while (futureEnergySurplus < energySurplusMax && l > interval) {
      //                    int finalL = l;
      //                    if (powerSetpoints[l] >= 0 &&
      // pastAndFutureIntervalPowerDeltas.stream().noneMatch(delta -> delta.key.equals(finalL))) {
      //
      //                        double surplusDeficit = energySurplusMax - futureEnergySurplus;
      //                        double intermediateEnergyCapacity = energyLevelMax -
      // energyLevelCalculator.apply(l);
      //                        double intermediatePowerCapacity = powerImportMaxCalculator.apply(l)
      // - powerSetpoints[l];
      //                        intermediatePowerCapacity = Math.min(intermediatePowerCapacity,
      // surplusDeficit / intervalSize);
      //                        intermediatePowerCapacity = Math.min(intermediatePowerCapacity,
      // intermediateEnergyCapacity / intervalSize);
      //
      //                        if (intermediatePowerCapacity > 0 && powerSetpoints[l] +
      // intermediatePowerCapacity > importCostAndPower[l][1] && costAndPower[0] +
      // importCostAndPower[l][0] < 0) {
      //                            // There is capacity and it is cost effective to use it
      //                            futureEnergySurplus = intermediatePowerCapacity * intervalSize;
      //                            pastAndFutureIntervalPowerDeltas.add(new Pair<>(l,
      // intermediatePowerCapacity));
      //                        }
      //                    }
      //                    l--;
      //                }
    ...
    if (energySurplus < energySurplusMax && importCostAndPower != null) {
      ...
      // in a cost effective way
      LOG.finest(
          "Looking for earlier import opportunities to maximise on this export opportunity: surplus="
              + energySurplus
              + ", max="
              + energySurplusMax);
      int i = interval - 1;
      ...
      while (energySurplus < energySurplusMax && j < pastOpportunities.size()) {
        ...
        double impPowerMax =
            Math.min(powerImportMaxCalculator.apply(interval), pastCostAndPower[2]);
        ...
        // We have spare energy capacity and power check if we don't violate energy max for any
        // future imports
        k = pastInterval;
        while (k < powerSetpoints.length && energySpace > 0 && energySpace >= energySpaceMin) {
          ...
          if (energySpace <= 0) {
            LOG.finest(
                "Earlier import opportunity would violate future energy max level: interval="
                    + j
                    + ", futureInterval="
                    + k);
          }
        ...
        if (impPowerCapacity > 0 && impPowerCapacity > pastCostAndPower[1]) {
          ...
          LOG.finest(
              "Earlier import opportunity identified: interval="
                  + pastInterval
                  + ", power="
                  + impPowerCapacity);
        }
    ...
    if (energySurplus > 0 && energySurplus >= energySurplusMin) {
      ...
      // Adjust past interval set points as required
      pastAndFutureIntervalPowerDeltas.forEach(
          intervalAndDelta -> powerSetpoints[intervalAndDelta.key] += intervalAndDelta.value);

      energySurplusMax = Math.min(energySurplusMax, energySurplus);
      powerCapacity =
          Math.max(expPowerMax - powerSetpoints[interval], -1d * (energySurplusMax / intervalSize));
      powerSetpoints[interval] = powerSetpoints[interval] + powerCapacity;
      LOG.finest(
          "Applied export earning opportunity: interval="
              + interval
              + ", set point="
              + powerSetpoints[interval]
              + " (delta: "
  ...
  /**
   * Returns a function that can be used to calculate any export saving (per kWh) and power band
   * needed to achieve it based on requested interval index and power export max value (negative as
   * this is for export). This is used to determine whether there are export opportunities for
   * earning/saving rather than using the grid.
   */
  public BiFunction<Integer, Double, double[]> getExportOptimiser(
      double[] powerNets,
      double[] powerNetLimits,
      double[] tariffImports,
      double[] tariffExports,
      double assetExportCost) {
  ...
  /**
   * Returns a function that can be used to calculate the optimum cost (per kWh) and power band
   * needed to achieve it based on requested interval index, power min and power max values. The
   * returned power band will satisfy the requested power min value but this could mean that cost is
   * not optimum for that interval if a lower power could be used. If possible a 0 power min should
   * be tried first for all applicable intervals and if more energy is required then another pass
   * can be made with a high enough min power to allow desired energy levels to be reached. This is
   * used to determine the best times and power values for importing energy to meet the
   * requirements.
   */
  public BiFunction<Integer, double[], double[]> getImportOptimiser(
      double[] powerNets,
      double[] powerNetLimits,
      double[] tariffImports,
      double[] tariffExports,
      double assetImportCost) {
    ...
    return (interval, powerRequiredMinMax) -> {
      ...
      // If export was paying then reducing export is a loss in earnings i.e. a cost and vice versa
      // hence the -1d
