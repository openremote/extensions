...
/**
 * Fills in power forecast from ForecastSolar (https://forecast.solar) for {@link
...
public class ForecastSolarService extends RouteBuilder implements ContainerService {
  ...
  protected static class EstimateResponse {
    @JsonProperty protected Result result;
  }

  protected static class Result {
    @JsonProperty protected Map<String, Double> wattHours;
    @JsonProperty protected Map<String, Double> wattHoursDay;
    @JsonProperty protected Map<String, Double> watts;
  }
  ...
  protected static final Logger LOG =
      SyslogCategory.getLogger(DATA, ForecastSolarService.class.getName());
  protected ResteasyWebTarget forecastSolarTarget;
  private String forecastSolarApiKey;
  private final Map<String, ElectricityProducerSolarAsset> electricityProducerSolarAssetMap =
      new HashMap<>();
  ...
  public void configure() throws Exception {
    from(PERSISTENCE_TOPIC)
        ...
        .process(
            exchange ->
                processAssetChange(
                    (PersistenceEvent<ElectricityProducerSolarAsset>)
  ...
  public void start(Container container) throws Exception {
    ...
    forecastSolarTarget =
        new WebTargetBuilder(
                getClient(),
                URI.create("https://api.forecast.solar/" + forecastSolarApiKey + "/estimate"))
            .build();
    ...
    List<ElectricityProducerSolarAsset> electricityProducerSolarAssets =
        assetStorageService
            .findAll(new AssetQuery().types(ElectricityProducerSolarAsset.class))
            .stream()
            .map(asset -> (ElectricityProducerSolarAsset) asset)
            .filter(
                electricityProducerSolarAsset ->
                    electricityProducerSolarAsset.isIncludeForecastSolarService().orElse(false))
            .toList();

    LOG.fine(
        "Number of electricity producer solar assets with forecast enabled = "
            + electricityProducerSolarAssets.size());

    for (ElectricityProducerSolarAsset electricityProducerSolarAsset :
        electricityProducerSolarAssets) {
      electricityProducerSolarAssetMap.put(
          electricityProducerSolarAsset.getId(), electricityProducerSolarAsset);
  ...
  protected synchronized void processElectricityProducerSolarAssetAttributeEvent(
    ...
    if (attributeName.equals(
        ElectricityProducerSolarAsset.SET_ACTUAL_SOLAR_VALUE_WITH_FORECAST.getName())) {
      ...
      ElectricityProducerSolarAsset asset =
          (ElectricityProducerSolarAsset) assetStorageService.find(attributeEvent.getId());

      if (asset != null && enabled) {
        assetProcessingService.sendAttributeEvent(
            new AttributeEvent(
                asset.getId(),
                ElectricityProducerSolarAsset.POWER,
                asset.getPowerForecast().orElse(null)),
    ...
    if (attributeName.equals(
        ElectricityProducerSolarAsset.INCLUDE_FORECAST_SOLAR_SERVICE.getName())) {
      boolean enabled = (Boolean) attributeEvent.getValue().orElse(false);

      if (enabled && !electricityProducerSolarAssetMap.containsKey(attributeEvent.getId())) {
        LOG.info(
            String.format(
                "Enabled solar forecast for ElectricityProducerSolarAsset: name='%s', ID='%s';",
        ...
        LOG.info(
            String.format(
                "Disabled solar forecast for ElectricityProducerSolarAsset: name='%s', ID='%s';",
                attributeEvent.getAssetName(), attributeEvent.getId()));
    ...
    if (attributeName.equals(ElectricityProducerSolarAsset.PANEL_AZIMUTH.getName())
      ...
      if (asset != null && asset.isIncludeForecastSolarService().orElse(false)) {
        ElectricityProducerSolarAsset assetPrevious =
            electricityProducerSolarAssetMap.get(asset.getId());

        String valueStr = attributeEvent.getValue().toString();
        String valuePreviousStr =
            assetPrevious
                .getAttributes()
                .get(attributeEvent.getName())
        ...
        if (!valueStr.equals(valuePreviousStr)) {
          ...
          if (attributeName.equals(ElectricityProducerSolarAsset.PANEL_AZIMUTH.getName())) {
            ...
            asset.setPanelPitch((Integer) value);
              ...
              ElectricityProducerSolarAsset.POWER_EXPORT_MAX.getName())) {
  ...
  protected void processAssetChange(
      PersistenceEvent<ElectricityProducerSolarAsset> persistenceEvent) {
    ...
    if (persistenceEvent.getCause() == PersistenceEvent.Cause.CREATE
      ...
      electricityProducerSolarAssetMap.put(
          persistenceEvent.getEntity().getId(), persistenceEvent.getEntity());
  ...
  protected void processSolarData() {
    ...
    if (currentMinute == 0) {
      electricityProducerSolarAssetMap.forEach(
          (assetId, electricityProducerSolarAsset) ->
              getSolarForecast(electricityProducerSolarAsset));
    }
  ...
  protected void getSolarForecast(ElectricityProducerSolarAsset electricityProducerSolarAsset) {
    ...
    if (lat.isEmpty() || lon.isEmpty() || pitch.isEmpty() || azimuth.isEmpty() || kwp.isEmpty()) {
      LOG.warning(
          String.format(
              ...
              electricityProducerSolarAsset.getName(),
              electricityProducerSolarAsset.getId(),
              lat,
              lon,
              azimuth,
              pitch,
    ...
    try (Response response =
        forecastSolarTarget
            .path(
                String.format(
                    "%f/%f/%d/%d/%f", lat.get(), lon.get(), pitch.get(), azimuth.get(), kwp.get()))
            .request()
            .build("GET")
            .invoke()) {
      if (response != null && response.getStatus() == 200) {
        ...
        if (responseModel != null) {
          ...
          List<ValueDatapoint> solarForecastListPrevious =
              assetPredictedDatapointService.getDatapoints(
                  new AttributeRef(
                      electricityProducerSolarAsset.getId(),
          ...
          for (ValueDatapoint datapoint : solarForecastListPrevious) {
            LocalDateTime dateTime =
                LocalDateTime.ofInstant(
                    Instant.ofEpochMilli(datapoint.getTimestamp()), ZoneId.systemDefault());
          ...
          String minKey =
              responseModel.result.watts.keySet().stream().min(String::compareTo).orElse("");
          String maxKey =
              responseModel.result.watts.keySet().stream().max(String::compareTo).orElse("");
          LocalDateTime startDateTime =
              LocalDateTime.parse(minKey, ISO_LOCAL_DATE_TIME_WITHOUT_T)
                  .toLocalDate()
                  .atStartOfDay();
          LocalDateTime endDateTime =
              LocalDateTime.parse(maxKey, ISO_LOCAL_DATE_TIME_WITHOUT_T)
                  .toLocalDate()
                  .plusDays(1)
                  .atStartOfDay();
          ...
          for (Map.Entry<String, Double> wattItem : responseModel.result.watts.entrySet()) {
            LocalDateTime dateTime =
                LocalDateTime.parse(wattItem.getKey(), ISO_LOCAL_DATE_TIME_WITHOUT_T);
          ...
          solarForecast.forEach(
              (dateTime, powerKiloWatt) -> {
                if (dateTime.isAfter(currentDateTime)
                    || solarForecastPrevious.get(dateTime) == null) {
                  ...
                  assetPredictedDatapointService.updateValue(
                      electricityProducerSolarAsset.getId(),
                      ElectricityProducerSolarAsset.POWER.getName(),
                      powerKiloWatt,
                      dateTime);
                }
              });
        }
        rulesService.fireDeploymentsWithPredictedDataForAsset(
            electricityProducerSolarAsset.getId());
      } else {
  ...
  protected void updateSolarForecastAttribute(
    ...
    List<ValueDatapoint<?>> solarForecastDatapoints =
    ...
    if (solarForecastDatapoints == null || solarForecastDatapoints.size() < 2) {
      LOG.warning(
          String.format(
              "ElectricityProducerSolarAsset: name='%s', ID='%s' doesn't have a solar forecast",
    ...
    ElectricityProducerSolarAsset asset =
        (ElectricityProducerSolarAsset)
            assetStorageService.find(electricityProducerSolarAsset.getId());
    long powerForecastAttributeTimeMillis =
        asset
            .getAttributes()
            .get(ElectricityProducerSolarAsset.POWER_FORECAST)
    ...
    if (solarForecastDatapointMin.getTimestamp() > powerForecastAttributeTimeMillis) {
      ...
      // Interpolate value
      double factor =
          (double) (currentTimeMillis - lowerTimestamp) / (upperTimestamp - lowerTimestamp);
