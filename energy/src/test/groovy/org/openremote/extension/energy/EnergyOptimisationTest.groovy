...
class EnergyOptimisationTest extends Specification {
  ...
  def "Check basic import optimiser"() {
    ...
    List<Pair<Integer, double[]>> optimisedPower = IntStream.range(0, intervalCount).mapToObj{
      new Pair<>(it, costCalculator.apply(it, [0d, powerImportMax] as double[]))
    }.collect()
    Collections.sort(optimisedPower, Comparator.comparingDouble({Pair<Integer, double[]> optimisedInterval ->
      optimisedInterval.value[0]
    }))
    ...
    optimisedPower = IntStream.range(0, intervalCount).mapToObj{
      new Pair<>(it, costCalculator.apply(it, [6d, powerImportMax] as double[]))
    }.collect()
    Collections.sort(optimisedPower, Comparator.comparingDouble({Pair<Integer, double[]> optimisedInterval ->
      optimisedInterval.value[0]
    }))
  ...
  def "Check basic export optimiser"() {
    ...
    List<Pair<Integer, double[]>> optimisedPower = IntStream.range(0, intervalCount).mapToObj{
      new Pair<>(it, costCalculator.apply(it, powerExportMax))
    }.collect()
    Collections.sort(optimisedPower, Comparator.comparingDouble({Pair<Integer, double[]> optimisedInterval ->
      optimisedInterval.value[0]
    }))
  ...
  def "Check energy schedule extraction"() {
    ...
    IntStream.range(1, energyMinLevels.length).mapToDouble{
      energyMinLevels[it]
    }.allMatch{
      it == 40d
    }
  ...
  def "Check full storage optimisation functionality"() {
    ...
    double[][] optimisedImport = IntStream.range(0, intervalCount).mapToObj{
      importCostCalculator.apply(it, [0d, powerImportMaxCalculator.apply(it)] as double[])
    }.toArray({
      new double[it][1]
    })
    ...
    double[][] optimisedExport = IntStream.range(0, intervalCount).mapToObj{
      exportCostCalculator.apply(it, powerExportMaxCalculator.apply(it))
    }.toArray({
      new double[it][1]
    })
    ...
    Function<Integer, Double> energyLevelCalculator = {int interval ->
      currentEnergyLevel + IntStream.range(0, interval).mapToDouble({j ->
        powerSetpoints[j] * intervalSize
      }).sum()
    }
