...
'''
  ...
  def "ENTSO-E integration test filters out points in the past when clock is mid-period"() {
    ...
    conditions.eventually {
      List<ValueDatapoint> datapoints = assetPredictedDatapointService.getDatapoints(attributeRef).sort {
        it.timestamp
      }
  ...
  def "ENTSO-E integration test writes predicted datapoints for 2 linked attributes with different zones"() {
    ...
    conditions.eventually {
      List<ValueDatapoint> beDatapoints = assetPredictedDatapointService.getDatapoints(beRef).sort {
        it.timestamp
      }
      List<ValueDatapoint> nlDatapoints = assetPredictedDatapointService.getDatapoints(nlRef).sort {
        it.timestamp
      }
  ...
  def "ENTSO-E integration test fetches a shared zone only once per polling cycle"() {
    ...
    conditions.eventually {
      List<ValueDatapoint> datapointsOne = assetPredictedDatapointService.getDatapoints(refOne).sort {
        it.timestamp
      }
      List<ValueDatapoint> datapointsTwo = assetPredictedDatapointService.getDatapoints(refTwo).sort {
        it.timestamp
      }

      assert datapointsOne.size() == 4
      assert datapointsTwo.size() == 4
      assert datapointsOne.collect {
        [it.timestamp, it.value]
      } == datapointsTwo.collect {
        [it.timestamp, it.value]
      }
